...
function fixFile(filePath: string): FixResult {
  try {
    ...
    content = content.replace(/from\s+["']@\/lib\/api-utils["']/g, 'from "@/lib/api-auth"');
    ...
    content = content.replace(/await\s+requireAuth\(\s*\)/g, 'await requireAuth(request)');

    // Step 6: Change apiResponse to apiSuccess
    content = content.replace(/return\s+apiResponse\(/g, 'return apiSuccess(');
