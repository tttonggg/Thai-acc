...
import { createWebhookEndpoint, getWebhookDeliveries, rotateWebhookSecret, deliverWebhook, WebhookEvents } from '@/lib/webhook-security'
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'trigger' && id) {
      ...
      if (!event) {
        return NextResponse.json(
          { success: false, error: 'Event required' },
          { status: 400 }
        )
      }
