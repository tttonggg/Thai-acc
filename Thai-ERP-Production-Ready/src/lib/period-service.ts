...
export async function generatePeriodReconciliationReport(
  ...
  for (const account of accounts) {
    ...
    const currentPeriodLines = lines.filter(
      (l) =>
        l.entry.date >= startDate &&
        l.entry.date <= endDate &&
        l.entry.status === "POSTED"
    )

    const currentDebitSum = currentPeriodLines.reduce(
      (sum, l) => sum + l.debit,
      0
    )
    const currentCreditSum = currentPeriodLines.reduce(
      (sum, l) => sum + l.credit,
      0
    )
  }
