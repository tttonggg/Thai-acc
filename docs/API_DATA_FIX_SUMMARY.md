...
**Date**: March 19, 2026 **Issue**: "Invalid purchases data format" error and
"Fetch failed" on multiple pages

---

## 🐛 Root Cause

API endpoints were returning **nested objects** (e.g., `vendor.name`) but
frontend components expected **flattened fields** (e.g., `vendorName`).
...
1. **Backend**: Used Prisma's `include` to fetch related objects (normalized
   data)
2. **Frontend**: Expected flattened fields for easier display (denormalized
   data)

**Solution**: Transform API responses to flatten nested objects before sending
to frontend.
...
✅ **COMPLETE** - All 3 APIs now transform data correctly to match frontend
expectations.
...
1. **Date Formatting**: All dates converted to ISO string format for consistent
   frontend handling
2. **Null Safety**: Used optional chaining (`?.`) and null coalescing (`|| ''`)
   to prevent errors
3. **Future**: Consider applying same pattern to other APIs (invoices, receipts,
   payments, etc.)
