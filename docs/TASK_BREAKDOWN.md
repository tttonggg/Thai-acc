...
> **Strategy:** Smallest possible independent tasks **Goal:** Minimize errors,
> enable incremental testing **Format:** Each task = 1 file change +
> verification
...
**File:** `src/stores/theme-store.ts` **Action:** Add `'professional'` to type
union **Lines:** +1 **Time:** 2 min
...
**Test:** File saves, no TypeScript errors **Dependencies:** None

---

### Task 1.2: Add professional theme colors object

**File:** `src/stores/theme-store.ts` **Action:** Add entry to `themeColors`
object **Lines:** +6 **Time:** 3 min
...
**Test:** File saves, no TypeScript errors **Dependencies:** Task 1.1

---

### Task 1.3: Add professional theme light mode CSS

**File:** `src/app/globals.css` **Action:** Add `[data-theme="professional"]`
block **Lines:** +40 **Time:** 10 min
...
**File:** `src/app/globals.css` **Action:** Add
`.dark[data-theme="professional"]` block **Lines:** +35 **Time:** 8 min
...
**File:** No change (verification only) **Action:** Test theme switcher UI
**Time:** 5 min
...
**File:** No change (verification only) **Action:** Manual smoke test **Time:**
10 min
...
**File:** No change (verification only) **Action:** Toggle dark mode, verify
**Time:** 5 min
...
**File:** Git **Action:** Commit professional theme **Time:** 2 min
...
**File:** `src/app/page.tsx` **Action:** Add Sheet import **Lines:** +1
**Time:** 2 min
...
**Test:** File saves, no import errors **Dependencies:** None

---

### Task 2.2: Add mobile menu state

**File:** `src/app/page.tsx` **Action:** Add useState for mobile menu **Lines:**
+1 **Time:** 2 min
...
**File:** `src/app/page.tsx` **Action:** Add menu button for mobile only
**Lines:** +20 **Time:** 10 min
...
**File:** `src/app/page.tsx` **Action:** Add Menu to lucide-react imports
**Lines:** +1 **Time:** 2 min
...
**Test:** File saves, no import errors **Dependencies:** Task 2.3

---

### Task 2.5: Add onCloseMobile prop to sidebar

**File:** `src/components/layout/keerati-sidebar.tsx` **Action:** Add optional
callback prop **Lines:** +3 **Time:** 3 min
...
**File:** `src/components/layout/keerati-sidebar.tsx` **Action:** Close drawer
on menu item click **Lines:** +1 **Time:** 3 min
...
**Test:** File saves **Dependencies:** Task 2.5

---

### Task 2.7: Hide sidebar on mobile, show on desktop

**File:** `src/app/page.tsx` **Action:** Add responsive classes to sidebar
wrapper **Lines:** Change 1 line **Time:** 2 min
...
**Test:** File saves **Dependencies:** None

---

### Task 2.8: Add top spacing for mobile menu button

**File:** `src/app/page.tsx` **Action:** Add padding-top on mobile only
**Lines:** Change 1 line **Time:** 2 min
...
**File:** No change (verification only) **Action:** Manual mobile test **Time:**
15 min
...
**File:** Git **Action:** Commit mobile drawer **Time:** 2 min
...
**File:** `src/components/invoices/invoice-list.tsx` **Action:** Wrap Table with
ScrollArea **Lines:** +3 **Time:** 3 min
...
**File:** `src/components/ar/customer-list.tsx` **Action:** Same as Task 3.1
**Lines:** +3 **Time:** 3 min
...
**File:** `src/components/ap/vendor-list.tsx` **Action:** Same as Task 3.1
**Lines:** +3 **Time:** 3 min
...
**File:** `src/components/products/products-page.tsx` **Action:** Same as Task
3.1 **Lines:** +3 **Time:** 3 min
...
**File:** `src/components/quotations/quotation-list.tsx` **Action:** Same as
Task 3.1 **Lines:** +3 **Time:** 3 min
...
**File:** `src/components/receipts/receipt-list.tsx` **Action:** Same as Task
3.1 **Lines:** +3 **Time:** 3 min
...
**File:** `src/components/payments/payment-list.tsx` **Action:** Same as Task
3.1 **Lines:** +3 **Time:** 3 min
...
**File:** `src/components/credit-notes/credit-note-list.tsx` **Action:** Same as
Task 3.1 **Lines:** +3 **Time:** 3 min
...
**File:** `src/components/debit-notes/debit-note-list.tsx` **Action:** Same as
Task 3.1 **Lines:** +3 **Time:** 3 min
...
**File:** `src/components/purchases/purchase-list.tsx` **Action:** Same as Task
3.1 **Lines:** +3 **Time:** 3 min
...
**Action:** Same as Task 3.1 **Lines:** +3 **Time:** 3 min
...
**File:** `src/components/purchase-orders/purchase-order-list.tsx` **Action:**
Same as Task 3.1 **Lines:** +3 **Time:** 3 min
...
**File:** `src/components/employees/employee-list.tsx` (if exists) **Action:**
Same as Task 3.1 **Lines:** +3 **Time:** 3 min
...
**File:** `src/components/inventory/warehouse-list.tsx` (if exists) **Action:**
Same as Task 3.1 **Lines:** +3 **Time:** 3 min
...
**File:** Git **Action:** Commit all table scroll changes **Time:** 5 min
...
**File:** `src/components/invoices/invoice-edit-dialog.tsx` **Action:** Add
responsive classes to DialogContent **Lines:** Change 1 line **Time:** 2 min
...
**File:** `src/components/ar/customer-edit-dialog.tsx` **Action:** Same as Task
4.1 **Time:** 2 min
...
**Files:** All edit/create dialog components **Action:** Same pattern **Time:**
2 min each
...
**File:** Git **Action:** Commit all dialog fixes **Time:** 5 min
...
**File:** `src/components/invoices/invoice-form.tsx` **Action:** Find and
replace grid classes **Lines:** Change multiple lines **Time:** 3 min
...
**Files:** All components with form grids **Action:** Same pattern **Time:** 3
min each
...
**File:** Git **Action:** Commit all form grid fixes **Time:** 5 min
...
**File:** `src/components/invoices/invoice-list.tsx` **Action:** Find all
buttons, check sizes **Time:** 5 min
...
**Files:** All components with buttons/links **Action:** Same pattern **Time:**
2-5 min per file
...
**File:** Git **Action:** Commit all touch target fixes **Time:** 5 min
...
**File:** `src/components/ui/status-badge.tsx` **Action:** New component file
**Lines:** +80 **Time:** 15 min
...
**Test:** Component renders without errors **Dependencies:** None

---

### Task 7.2-7.20: Replace inline badges with StatusBadge

**Files:** All components with status badges **Action:** Find and replace
hardcoded badge styles **Time:** 3 min each
...
**File:** `e2e/contrast-audit.spec.ts` **Action:** New test file **Lines:** +50
**Time:** 20 min
  ...
  themes.forEach((theme) => {
    modes.forEach((mode) => {
      test(`contrast: ${theme} theme in ${mode} mode`, async ({ page }) => {
        ...
        const violations = await page.accessibility
          .scan()
          .then((r) => r.violations);
...
**Test:** Run `bun run test:contrast` **Dependencies:** None

---

### Task 8.2: Manual mobile test checklist

**File:** No change (testing only) **Action:** Test all phases on real device
**Time:** 30 min
...
**File:** `CLAUDE.md` **Action:** Document new theme and mobile features
**Lines:** +20 **Time:** 10 min
...
**File:** No change (testing only) **Action:** Run full test suite **Time:** 20
min
...
**File:** Git **Action:** Commit tests and docs **Time:** 2 min
