...
**Last Updated**: 2026-04-16 **Status**: ✅ Both issues fixed and deployed
...
The `/api/csrf/token` endpoint requires authentication, but was returning
**500** instead of **401** on auth failure. This created two problems:
...
3. If not authenticated: `requireAuth()` throws "ไม่ได้รับอนุญาต -
   กรุณาเข้าสู่ระบบ"
...
- ✅ Auth failures log as `console.warn` (expected), system errors as
...
**Also affected**: Summary cards showing fractional Baht (฿1,234.567 instead of
฿1,235)

### Root Cause Analysis

**The Problem**: **Double conversion** - API converts Satang → Baht, but
frontend divides by 100 again.
...
3. ✅ **Added** fraction digits:
   `{ minimumFractionDigits: 0, maximumFractionDigits: 0 }`
4. ✅ **Rounded** to whole Baht for summary cards (large totals don't need
   decimals)
...
**Note**: `formatBaht()` helper from `/src/lib/currency.ts` expects Satang
input, so don't use it for API response values (they're already in Baht).
...
- `/src/components/invoices/invoice-list.tsx` - ✅ Fixed display (removed
  `/ 100`)
...
5. ✅ Monitor logs for CSRF errors:
   `ssh root@VPS "tail -f /root/thai-acc/server.log"`
...
1. ✅ **Always check HTTP status codes** - 401 (auth) vs 500 (server) vs 403
   (forbidden)
...
- [ ] Fix `/ 100` bug in other components (credit-notes, debit-notes, receipts,
      payments)
...
**Status**: ✅ Both fixes deployed to production **Last Tested**: 2026-04-16
**Monitor**: Check logs for CSRF errors and currency display issues for next 48
hours
