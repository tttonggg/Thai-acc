...
Successfully standardized status badge colors and variants across all modules in
the Thai Accounting ERP system. This ensures consistent visual language
...
6. **Purchase Orders**
   (`src/components/purchase-orders/purchase-order-list.tsx`)
   - Standardized status badges (DRAFT, PENDING, APPROVED, ORDERED, RECEIVING,
     RECEIVED, CANCELLED)
...
8. **Quotations** (`src/components/quotations/quotation-list.tsx`)
   - Standardized DRAFT, SENT, APPROVED, REJECTED, REVISED, EXPIRED, CONVERTED,
     CANCELLED badges
...
- `/Users/tong/Thai-acc/src/lib/status-badge.tsx` - Centralized status badge
  utility
...
2. **Status Transitions**: Test that status changes properly update badge
   appearance
...
All status badges now follow a unified design system, making the application
