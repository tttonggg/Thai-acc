...
**Problem**: APIs were returning `{data: {data: [...], pagination: {...}}}`
...
**Solution**: Changed from `apiResponse({data: result, pagination})` to direct
`Response.json({success: true, data: result, pagination})`

### 2. Database Connection Issue (Standalone Build)

**Problem**: Standalone server was using `.next/standalone/prod.db` (empty
database) instead of `prisma/dev.db`

**Root Cause**: `.env.production` in standalone directory pointed to wrong
database path
...
**Solution**: Changed from `await requireAuth()` to `await requireAuth(request)`
in all fixed API files
...
2. Copy Prisma client if needed:
