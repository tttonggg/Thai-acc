...
**Audit Date**: 2026-04-14 **Auditor**: Claude Code (Frontend Engineer Agent)
...
✅ **GOOD NEWS**: All forms are **CORRECTLY** handling currency inputs. No
critical bugs found.
...
- Document totals: `subtotal`, `vatAmount`, `grandTotal`, `withholdingAmount`,
  `netTotal`
...
<Input
  ...
  onChange={(e) =>
    field.onChange(Math.round(parseFloat(e.target.value) * 100) || 0)
  } // ✅ ×100
/>
```
...
<Input
  ...
  value={
    allocations.find((a) => a.invoiceId === invoice.id)?.amount
      ? allocations.find((a) => a.invoiceId === invoice.id)!.amount / 100
      : ''
  } // ✅ Displays Baht
  onChange={(e) => {
...
<Input
  ...
  {...field}
  onChange={
    (e) => field.onChange(parseFloat(e.target.value) || 0) // ✅ No division
  }
/>
```
...
**Pattern A: Direct Baht Storage (Invoice, Purchase, Credit Note, Debit Note,
Quotation)**
...
<Input
  ...
  onChange={(e) => updateLine(id, 'unitPrice', parseFloat(e.target.value) || 0)}
/>;
...
test('currency input stores Baht, converts to Satang for API', async () => {
  ...
  expect(line.unitPrice).toBe(100.5);
...
Forms store currency in **Baht** (float) for display and calculation. Convert to
**Satang** (integer × 100) at API boundary.
...
✅ **All forms pass the audit**. Currency input handling is correct across the
entire codebase.

**No bugs found.** The codebase follows consistent patterns for currency
conversion between Baht (display/form) and Satang (database/API).
...
**Audit completed**: 2026-04-14 **Files audited**: 7 forms **Bugs found**: 0
**Critical issues**: 0
