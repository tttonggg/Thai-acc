...
- Prisma's `findMany({ include: { vendor: {...} } })` crashes on null
  relationships
...
**Preemptive Fix**: Applied the same two-step query pattern to prevent the same
crash
...
2. **Manual database edits** - Direct database changes can break referential
   integrity
3. **Cascade deletes not working** - Vendors/customers deleted without updating
   related records
...
The dev server will hot-reload these changes automatically. If you still see
errors:

1. **Hard refresh your browser**: `Ctrl + Shift + R` (Windows/Linux) or
   `Cmd + Shift + R` (Mac)
