...
test.describe('Credit Notes & Debit Notes - Create Form Test', () => {
    ...
    if (!currentUrl.includes('/dashboard') && !currentUrl.includes('login')) {
      ...
      if (isVisible) {
        ...
        const passwordInput = page
          .locator('input[type="password"], input[name="password"], input[placeholder*="•••"]')
          .first();
        await passwordInput.fill('admin123');

        // Click login button
        const loginButton = page
          .locator('button:has-text("เข้าสู่ระบบ"), button[type="submit"]')
          .first();
  ...
  test('should open Debit Note form without errors', async ({ page }) => {
    ...
    const productFilterErrors = errors.filter((e) =>
      e.includes('products.filter is not a function')
    );
