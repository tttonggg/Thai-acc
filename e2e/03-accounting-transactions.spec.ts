...
const INVOICE_TEST_DATA = {
  ...
  items: [
    {
      productCode: 'PROD001',
      productName: 'สินค้าทดสอบ รุ่น 1',
      quantity: 10,
      unitPrice: 1000,
      vatRate: 7,
    },
...
async function verifyJournalEntry(page, description) {
  ...
  const searchInput = page.locator(
    'input[placeholder*="ค้นหา"], input[placeholder*="search"], [data-testid="search"]'
  );
...
test.describe('Phase 3: Invoice Creation & GL Posting (ใบกำกับภาษี)', () => {
  ...
  test('[CREATE] Create sales invoice with multiple line items', async ({ page }) => {
    ...
    const item1Price = page
      .locator('[data-testid="line-0-price"], [name="lines.0.unitPrice"]')
      .first();
    ...
    await page.click(
      'button:has-text("เพิ่มรายการ"), button:has-text("Add Item"), button:has-text("+")'
    );
    await page.waitForTimeout(300);

    const item2Qty = page.locator('[data-testid="line-1-qty"], [name="lines.1.quantity"]').first();
    const item2Price = page
      .locator('[data-testid="line-1-price"], [name="lines.1.unitPrice"]')
      .first();
    ...
    // Submit invoice
    await page.click(
      'button:has-text("บันทึก"), button:has-text("保存"), button:has-text="Create"'
    );

    // Verify success
    await expect(
      page.locator('text=บันทึกสำเร็จ, text=สำเร็จ').or(page.locator('text=Success'))
    ).toBeVisible({ timeout: 5000 });

    console.log('✅ [CREATE] Invoice created with multiple line items');

    await page.screenshot({
      path: 'test-results/evidence/03-invoice-create-success.png',
      fullPage: true,
    });
  });
  ...
  test('[WORKFLOW] Invoice lifecycle: Draft → Issued → Paid', async ({ page }) => {
    ...
    // Issue the invoice
    await page.click(
      'button:has-text("ออกใบกำกับภาษี"), button:has-text("Issue"), button:has-text="Post"'
    );
    await page.waitForTimeout(1000);
...
test.describe('Phase 3: Manual Journal Entry & Double-Entry Validation', () => {
  ...
  test('[CREATE] Create manual journal entry', async ({ page }) => {
    ...
    await page.fill(
      ...
      'ทดสอบบันทึกบัญชี manually'
    );

    // Add debit line
    const debitAccount = page
      .locator('[name="lines.0.accountId"], [data-testid="line-0-account"]')
      .first();
    const debitAmount = page
      .locator('[name="lines.0.debit"], [data-testid="line-0-debit"]')
      .first();
    ...
    const creditAccount = page
      .locator('[name="lines.1.accountId"], [data-testid="line-1-account"]')
      .first();
    const creditAmount = page
      .locator('[name="lines.1.credit"], [data-testid="line-1-credit"]')
      .first();
  ...
  test('[VALIDATION] Verify double-entry balance (Debit = Credit)', async ({ page }) => {
    ...
    const errorMessage = page.locator(
      'text=ไม่สมดุล, text=Debit must equal Credit, text=ต้องสมดุล'
    );
