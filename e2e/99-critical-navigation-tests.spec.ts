...
test.describe('Critical Navigation & Redirect Tests', () => {
  ...
  test('[CRITICAL] Should NOT redirect to dashboard when accessing Credit Notes', async ({
    page,
  }) => {
    await loginAsAdmin(page);
  ...
  test('[CRITICAL] Should NOT redirect to dashboard when accessing Debit Notes', async ({
    page,
  }) => {
    await loginAsAdmin(page);
  ...
  test('[CRITICAL] Should NOT redirect to dashboard when accessing Customers (AR)', async ({
    page,
  }) => {
    await loginAsAdmin(page);
  ...
  test('[CRITICAL] Should NOT redirect to dashboard when accessing Vendors (AP)', async ({
    page,
  }) => {
    await loginAsAdmin(page);
...
test.describe('Document Type Filtering Tests', () => {
  ...
  test('[FILTERING] Invoices section should NOT show Credit Notes or Debit Notes', async ({
    page,
  }) => {
    // Navigate to invoices
    const invoiceButton = page.locator('aside nav button').filter({ hasText: 'ใบกำกับภาษี' });
