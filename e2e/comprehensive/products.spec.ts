...
test.describe('Products - Comprehensive Tests', () => {
  ...
  test('[NAVIGATE] To Products module', async ({ page }) => {
    ...
    // Verify page title
    const title = page.locator('h1:has-text("สินค้า"), h1:has-text("Products"), h1:has-text("สินค้าและบริการ")')
  ...
  test('[CREATE] Fill and submit new product form', async ({ page }) => {
    ...
    const addBtn = page.locator('button:has-text("เพิ่มสินค้า"), button:has-text("Add Product"), button:has-text("เพิ่ม")').first()
    await addBtn.click()
    ...
    const inventoryCheckbox = page.locator('input[type="checkbox"][name*="inventory"], label:has-text("สินค้าคงเหลือ") input').first()
  ...
  test('[EDIT] Product button per row', async ({ page }) => {
    ...
    // Edit product price
    const priceInput = page.locator('input[name*="price"], input[name*="ราคา"], label:has-text("ราคาขาย") + input').first()
    ...
    for (let i = 0; i < rowCount; i++) {
      ...
      if (text && text.includes(testProductCode)) {
        ...
        if (await editBtn.isVisible().catch(() => false)) {
          ...
          // Change VAT type
          const vatSelect = page.locator('select[name*="vat"], label:has-text("ภาษีมูลค่าเพิ่ม") + select').first()
  ...
  test('[VERIFY] Stock level indicator', async ({ page }) => {
    ...
    for (let i = 0; i < rowCount; i++) {
      ...
      if (text && text.includes(testProductCode)) {
        ...
        const stockCell = row.locator('td:has-text("ชิ้น"), td:has-text("stock"), td[class*="stock"]').first()
  ...
  test('[FILTER] Product by category', async ({ page }) => {
    ...
    const categoryFilter = page.locator('select[name*="category"], [role="combobox"], button:has-text("หมวดหมู่"), button:has-text("ประเภท")').first()
  ...
  test('[FILTER] Product by status', async ({ page }) => {
    ...
    const statusFilter = page.locator('select[name*="status"], [role="combobox"], button:has-text("สถานะ")').first()
  ...
  test('[INTEGRATION] Product appears in invoice line items', async ({ page }) => {
    ...
    const productSelect = page.locator('select[name*="product"], [role="combobox"]:has-text("สินค้า"), label:has-text("สินค้า") + select').first()
    ...
    if (await productSelect.isVisible().catch(() => false)) {
      ...
      const testProductOption = page.locator(`[role="option"]:has-text("${testProductCode}"), [role="option"]:has-text("${testProductData.name}"), option[value*="${testProductCode}"]`).first()
  ...
  test('[VERIFY] Price and cost display in table', async ({ page }) => {
    ...
    const priceHeader = table.locator('th:has-text("ราคาขาย"), th:has-text("Price"), th:has-text("ราคา")').first()
