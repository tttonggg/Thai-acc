...
test.describe('Vendors - Comprehensive Tests', () => {
  ...
  test('[NAVIGATE] To Vendors module', async ({ page }) => {
    ...
    const title = page.locator(
      'h1:has-text("เจ้าหนี้"), h1:has-text("Vendors"), h1:has-text("ผู้ขาย")'
    );
  ...
  test('[CREATE] Fill and submit new vendor form', async ({ page }) => {
    ...
    const addBtn = page
      .locator(
        'button:has-text("เพิ่มผู้ขาย"), button:has-text("Add Vendor"), button:has-text("เพิ่ม")'
      )
      .first();
    await addBtn.click();
  ...
  test('[VIEW] Vendor details', async ({ page }) => {
    ...
    for (let i = 0; i < rowCount; i++) {
      ...
      if (text && text.includes(testVendorCode)) {
        ...
        const viewBtn = row
          .locator(
            'button[aria-label*="ดู"], button[title*="ดู"], button svg[class*="eye"], a[href*="/vendors/"]'
          )
          .first();
  ...
  test('[VERIFY] Vendor balance display', async ({ page }) => {
    ...
    const balanceHeader = table
      .locator('th:has-text("ยอดค้างจ่าย"), th:has-text("Balance"), th:has-text("หนี้")')
      .first();
  ...
  test('[INTEGRATION] Vendor appears in purchase dropdown', async ({ page }) => {
    ...
    const vendorSelect = page
      .locator(
        'select[name*="vendor"], [role="combobox"]:has-text("ผู้ขาย"), label:has-text("ผู้ขาย") + select, label:has-text("ผู้ขาย") + div'
      )
      .first();
    ...
    if (await vendorSelect.isVisible().catch(() => false)) {
      ...
      const testVendorOption = page
        .locator(
          `[role="option"]:has-text("${testVendorCode}"), [role="option"]:has-text("${testVendorData.name}"), option[value*="${testVendorCode}"]`
        )
        .first();
