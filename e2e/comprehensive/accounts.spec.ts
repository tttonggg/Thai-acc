...
test.describe('Chart of Accounts - Comprehensive Tests', () => {
  ...
  test('[EDIT] Account button per row', async ({ page }) => {
    ...
    for (let i = 0; i < rowCount; i++) {
      ...
      if (text && text.includes(testAccountCode)) {
        ...
        const editBtn = row
          .locator('button[aria-label*="แก้ไข"], button[title*="แก้ไข"], button svg[class*="edit"]')
          .first();
    ...
    const nameInput = page
      .locator(
        'input[name*="name"], label:has-text("ชื่อบัญชี") + input, label:has-text("ชื่อบัญชี") + div input'
      )
      .first();
  ...
  test('[CREATE] Add child account button', async ({ page }) => {
    ...
    for (let i = 0; i < rowCount; i++) {
      ...
      if (text && text.includes(testAccountCode)) {
        ...
        const addChildBtn = row
          .locator(
            'button:has-text("เพิ่มบัญชีลูก"), button:has-text("Add Child"), [aria-label*="child"]'
          )
          .first();
      ...
      if (text && text.includes(testChildAccountCode)) {
        ...
        if (await deleteBtn.isVisible().catch(() => false)) {
          ...
          const confirmBtn = confirmDialog
            .locator('button:has-text("ลบ"), button:has-text("ยืนยัน"), button:has-text("Confirm")')
            .first();
  ...
  test('[VALIDATE] Cannot delete account with transactions', async ({ page }) => {
    ...
    for (let i = 0; i < Math.min(rowCount, 50); i++) {
      ...
      if (text && text.includes('1110')) {
        ...
        const deleteBtn = row
          .locator('button svg[class*="trash"], button svg[class*="delete"]')
          .first();
        ...
        if (await deleteBtn.isVisible().catch(() => false)) {
          ...
          const error = page
            .locator('text=ไม่สามารถลบ, text=ไม่ได้, text=มีรายการ, [role="alert"]')
            .first();
          const errorVisible = await error.isVisible().catch(() => false);
  ...
  test('[FILTER] Account by type', async ({ page }) => {
    ...
    const typeFilter = page
      .locator('select[name*="type"], [role="combobox"], button:has-text("ประเภท")')
      .first();
    ...
    if (await typeFilter.isVisible().catch(() => false)) {
      ...
      const assetOption = page
        .locator(
          '[role="option"]:has-text("ASSET"), [role="option"]:has-text("สินทรัพย์"), text=ASSET'
        )
        .first();
  ...
  test('[PAGINATION] Navigate through account pages', async ({ page }) => {
    ...
    const nextPageBtn = page
      .locator(
        'button:has-text("ถัดไป"), button:has-text("Next"), [aria-label="Next"], [data-testid*="next"]'
      )
      .first();
    const prevPageBtn = page
      .locator(
        'button:has-text("ก่อนหน้า"), button:has-text("Prev"), [aria-label="Previous"], [data-testid*="prev"]'
      )
      .first();
  ...
  test('[UI] Expand and collapse account hierarchy', async ({ page }) => {
    ...
    const expandBtn = page
      .locator(
        'button[aria-label*="expand"], button[aria-label*="collapse"], button svg[class*="chevron"], button[class*="tree"]'
      )
      .first();
    ...
    if (await expandBtn.isVisible().catch(() => false)) {
      ...
      console.log(
        `✅ Expanded account hierarchy (rows: ${initialRowCount} -> ${expandedRowCount})`
      );
      ...
      console.log(
        `✅ Collapsed account hierarchy (rows: ${expandedRowCount} -> ${collapsedRowCount})`
      );
    } else {
