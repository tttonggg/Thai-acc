...
test.describe('Customers - Comprehensive Tests', () => {
  ...
  test('[NAVIGATE] To Customers module', async ({ page }) => {
    ...
    // Verify page title
    const title = page.locator('h1:has-text("ลูกหนี้"), h1:has-text("Customers"), h1:has-text("ลูกค้า")')
  ...
  test('[CREATE] Fill and submit new customer form', async ({ page }) => {
    ...
    const addBtn = page.locator('button:has-text("เพิ่มลูกค้า"), button:has-text("Add Customer"), button:has-text("เพิ่ม")').first()
    await addBtn.click()
  ...
  test('[VIEW] Customer details', async ({ page }) => {
    ...
    for (let i = 0; i < rowCount; i++) {
      ...
      if (text && text.includes(testCustomerCode)) {
        ...
        const viewBtn = row.locator('button[aria-label*="ดู"], button[title*="ดู"], button svg[class*="eye"], a[href*="/customers/"]').first()
        ...
        if (await viewBtn.isVisible().catch(() => false)) {
          ...
          if (detailsVisible) {
            ...
            const hasName = await detailsPanel.locator(`text=${testCustomerData.name}`).count() > 0
  ...
  test('[VERIFY] Customer balance display', async ({ page }) => {
    ...
    const table = page.locator('table').first()
    const balanceHeader = table.locator('th:has-text("ยอดค้างชำระ"), th:has-text("Balance"), th:has-text("หนี้")').first()
  ...
  test('[INTEGRATION] Customer appears in invoice dropdown', async ({ page }) => {
    ...
    const customerSelect = page.locator('select[name*="customer"], [role="combobox"]:has-text("ลูกค้า"), label:has-text("ลูกค้า") + select, label:has-text("ลูกค้า") + div').first()
    ...
    if (await customerSelect.isVisible().catch(() => false)) {
      ...
      const testCustomerOption = page.locator(`[role="option"]:has-text("${testCustomerCode}"), [role="option"]:has-text("${testCustomerData.name}"), option[value*="${testCustomerCode}"]`).first()
