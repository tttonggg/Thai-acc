...
const TEST_USERS = [
  ...
  { email: 'accountant@thaiaccounting.com', password: 'acc123', role: 'ACCOUNTANT', name: 'นักบัญชี ทดสอบ' },
  { email: 'user@thaiaccounting.com', password: 'user123', role: 'USER', name: 'ผู้ใช้ทั่วไป' },
  { email: 'viewer@thaiaccounting.com', password: 'viewer123', role: 'VIEWER', name: 'ผู้ชมเท่านั้น' },
]
...
test.describe('Phase 1: Authentication & Login Tests', () => {
  ...
  for (const user of TEST_USERS) {
    test(`[${user.role}] Login successfully and verify dashboard access`, async ({ page }) => {
      ...
      await expect(page.locator('h1:has-text("ภาพรวมธุรกิจ")').or(page.locator('nav, aside').first()).or(page.locator('.dashboard, [data-testid="dashboard"]')).first()).toBeVisible({ timeout: 10000 })
  ...
  test('[NEGATIVE] Should show error with invalid email', async ({ page }) => {
    ...
    await expect(page.locator('[role="alert"], .alert, [data-testid="error"], .destructive').filter({ hasText: 'อีเมลหรือรหัสผ่านไม่ถูกต้อง' }).or(page.locator('text=อีเมลหรือรหัสผ่านไม่ถูกต้อง')).first()).toBeVisible({ timeout: 5000 })
  ...
  test('[NEGATIVE] Should show error with invalid password', async ({ page }) => {
    ...
    await expect(page.locator('[role="alert"], .alert, [data-testid="error"], .destructive').filter({ hasText: /อีเมล.*ไม่ถูกต้อง/ }).or(page.locator('text=ไม่ถูกต้อง')).first()).toBeVisible({ timeout: 5000 })
  ...
  test('[SESSION] Should persist session across page reloads', async ({ page }) => {
    ...
    await expect(page.locator('h1:has-text("ภาพรวมธุรกิจ")').or(page.locator('nav, aside').first()).or(page.locator('.dashboard')).first()).toBeVisible({ timeout: 10000 })

    // Reload the page 3 times to verify session persistence
    for (let i = 1; i <= 3; i++) {
      await page.reload()
      await expect(page.locator('h1:has-text("ภาพรวมธุรกิจ")').or(page.locator('nav, aside').first()).or(page.locator('.dashboard')).first()).toBeVisible({ timeout: 5000 })
  ...
  test('[SESSION] Should handle multiple tabs with shared session', async ({ context }) => {
    ...
    await expect(page1.locator('h1:has-text("ภาพรวมธุรกิจ")').or(page1.locator('nav, aside')).first()).toBeVisible({ timeout: 10000 })
    ...
    const dashboardVisible2 = await page2.locator('h1:has-text("ภาพรวมธุรกิจ")').or(page2.locator('nav, aside')).first().isVisible({ timeout: 5000 }).catch(() => false)
    ...
    await page3.goto('/')
    const dashboardVisible3 = await page3.locator('h1:has-text("ภาพรวมธุรกิจ")').or(page3.locator('nav, aside')).first().isVisible({ timeout: 5000 }).catch(() => false)
