...
test.describe('Phase 4: Chart of Accounts Validation (ผังบัญชี)', () => {
  ...
  test('[VALIDATE] Verify hierarchy and parent-child relationships', async ({ page }) => {
    ...
    const expandButtons = page.locator(
      'button[aria-expanded], .expand-icon, [data-testid="expand"]'
    );
...
test.describe('Phase 4: Financial Reports - Trial Balance (งบทดลอง)', () => {
  ...
  test('[REPORT] Generate trial balance report', async ({ page }) => {
    ...
    await page.screenshot({
      path: 'test-results/evidence/04-report-trial-balance.png',
      fullPage: true,
    });
  });
  ...
  test('[VALIDATION] Verify trial balance totals match', async ({ page }) => {
    ...
    if (
      (await debitTotal.isVisible({ timeout: 3000 })) &&
      (await creditTotal.isVisible({ timeout: 3000 }))
    ) {
...
test.describe('Phase 4: Financial Reports - Balance Sheet & P&L', () => {
  ...
  test('[REPORT] Generate Balance Sheet (งบดุล)', async ({ page }) => {
    ...
    console.log(
      `   Assets: ${(await assets.isVisible({ timeout: 2000 }).catch(() => false)) ? '✓' : '✗'}`
    );
    console.log(
      `   Liabilities: ${(await liabilities.isVisible({ timeout: 2000 }).catch(() => false)) ? '✓' : '✗'}`
    );
    console.log(
      `   Equity: ${(await equity.isVisible({ timeout: 2000 }).catch(() => false)) ? '✓' : '✗'}`
    );
    ...
    await page.screenshot({
      path: 'test-results/evidence/04-report-balance-sheet.png',
      fullPage: true,
    });
  });
  ...
  test('[REPORT] Generate Income Statement (งบกำไรขาดทุน)', async ({ page }) => {
    ...
    console.log(
      `   Revenue: ${(await revenue.isVisible({ timeout: 2000 }).catch(() => false)) ? '✓' : '✗'}`
    );
    console.log(
      `   Expenses: ${(await expenses.isVisible({ timeout: 2000 }).catch(() => false)) ? '✓' : '✗'}`
    );
    console.log(
      `   Net Profit: ${(await profit.isVisible({ timeout: 2000 }).catch(() => false)) ? '✓' : '✗'}`
    );

    await page.screenshot({
      path: 'test-results/evidence/04-report-income-statement.png',
      fullPage: true,
    });
  });
});
...
test.describe('Phase 4: VAT Report & Tax Compliance', () => {
  ...
  test('[REPORT] Generate VAT report (ภาษีมูลค่าเพิ่ม)', async ({ page }) => {
    ...
    console.log(
      `   VAT Output (Sales): ${(await vatOutput.isVisible({ timeout: 2000 }).catch(() => false)) ? '✓' : '✗'}`
    );
    console.log(
      `   VAT Input (Purchases): ${(await vatInput.isVisible({ timeout: 2000 }).catch(() => false)) ? '✓' : '✗'}`
    );
    console.log(
      `   Net VAT Payable: ${(await vatNet.isVisible({ timeout: 2000 }).catch(() => false)) ? '✓' : '✗'}`
    );
...
test.describe('Phase 4: Database Relationship Validation', () => {
  ...
  test('[INTEGRITY] Verify customer → invoice → receipt chain', async ({ page }) => {
    ...
    if (hasInvoices) {
      ...
      const invoiceDetails = page.locator(
        '[role="dialog"], .modal, [data-testid="invoice-details"]'
      );
      if (await invoiceDetails.isVisible({ timeout: 3000 })) {
        ...
        const hasAppliedReceipts = await appliedReceipts
          .isVisible({ timeout: 2000 })
          .catch(() => false);
  ...
  test('[CONSISTENCY] Verify account balances across modules', async ({ page }) => {
    ...
    if (await searchInput.isVisible({ timeout: 3000 })) {
      ...
      if (await cashAccount.isVisible({ timeout: 2000 })) {
        ...
        console.log(
          `✅ [CONSISTENCY] Cash account from COA: ${(balanceText || '').substring(0, 100)}`
        );
      }
    }
