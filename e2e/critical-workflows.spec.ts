...
test.describe('CRITICAL-001: Invoice Workflow (ใบกำกับภาษี)', () => {
  ...
  test('[STEP 1] Create draft invoice', async ({ page }) => {
    ...
    await page.waitForFunction(() => (window as any).__zustand?.getState?.()?.currentModule !== 'dashboard', { timeout: 10000 }).catch(() => {})
    ...
    const saveBtn = page.locator('button').filter({ hasText: /บันทึก/ }).last()
  ...
  test('[STEP 2] Verify draft invoices exist', async ({ page }) => {
    ...
    const response = await page.request.get('http://localhost:3002/api/invoices?status=DRAFT&limit=5')
    if (response.ok()) {
      const data = await response.json()
  ...
  test('[STEP 3] Issue invoice and verify JE created', async ({ page }) => {
    ...
    const draftResponse = await page.request.get('http://localhost:3002/api/invoices?status=DRAFT&limit=5')
    ...
    if (draftInvoice) {
      const issueResponse = await page.request.post(`http://localhost:3002/api/invoices/${draftInvoice.id}`, {
        data: { action: 'post' }
      })
      ...
      if (issueResponse.ok()) {
        ...
        if (issued.data?.journalEntryId) {
          const jeResponse = await page.request.get(`http://localhost:3002/api/journal/${issued.data.journalEntryId}`)
          if (jeResponse.ok()) {
        ...
        const getResponse = await page.request.get(`http://localhost:3002/api/invoices/${draftInvoice.id}`)
        if (getResponse.ok()) {
...
test.describe('CRITICAL-002: Credit Note Workflow (ใบลดหนี้)', () => {
  ...
  test('[STEP 1] Create credit note against invoice', async ({ page }) => {
    ...
    const invoicesResponse = await page.request.get('http://localhost:3002/api/invoices?status=ISSUED&limit=3')
  ...
  test('[STEP 3] Verify journal entry created', async ({ page }) => {
    ...
    await page.evaluate(() => { window.history.pushState({}, '', '/journal') })
    await page.waitForTimeout(2000)
...
test.describe('CRITICAL-004: Journal Entry Workflow (บันทึกบัญชี)', () => {
    ...
    if (response.ok()) {
      ...
      for (const entry of entries) {
        if (entry.totalDebit !== entry.totalCredit) {
          console.log(`⚠️ JE ${entry.entryNo} unbalanced: Dr=${entry.totalDebit} Cr=${entry.totalCredit}`)
        }
      }
  ...
  test('[STEP 4] Verify posted journal entries', async ({ page }) => {
    ...
    if (response.ok()) {
      ...
      console.log(`✅ Journal entries: ${posted.length} posted of ${(data.data || []).length} total`)
    }
  })
})
