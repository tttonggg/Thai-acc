...
test.describe('Purchase Request & Purchase Order Workflow', () => {
  ...
  test('[PO-001] Create Purchase Order', async () => {
    ...
    const vendorName = await page.locator(`select[name="vendorId"] option[value="${testData.vendorIds[0]}"]`).textContent();
  ...
  test('[PO-005] Receive Purchase Order', async () => {
    ...
    // Receive PO items
    await poPage.receivePO(po.orderNo, [
      { receivedQty: 10, notes: 'รับครบตามจำนวน' },
    ]);
  ...
  test('[ERROR-001] Create PR without line items', async () => {
    ...
    expect(dialogVisible || await page.locator('[data-sonner-toast]:has-text("รายการ")').isVisible().catch(() => false)).toBeTruthy();
  });
  ...
  test('[ERROR-002] Create PO without vendor', async () => {
    ...
    const errorVisible = await page.locator('[data-sonner-toast]:has-text("ผู้ขาย"), [data-sonner-toast]:has-text("vendor"), [data-sonner-toast]:has-text("กรอก"]').isVisible().catch(() => false);
