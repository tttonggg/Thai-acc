...
test.describe('Phase 2: Master Data Creation - Customers (ลูกหนี้)', () => {
  ...
  test('[CREATE] Create first customer with complete data', async ({ page }) => {
    ...
    // Click create button
    await page.click(
      'button:has-text("เพิ่มลูกค้า"), button:has-text("สร้างใหม่"), button:has-text("สร้าง")'
    );

    // Fill customer form
    await page.fill(
      '[name="code"], [data-testid="code"], input[placeholder*="รหัส"]',
      TEST_CUSTOMERS[0].code
    );
    ...
    await page.fill(
      '[name="subDistrict"], [data-testid="subDistrict"]',
      TEST_CUSTOMERS[0].subDistrict
    );
    ...
    await page.fill(
      '[name="postalCode"], [data-testid="postalCode"]',
      TEST_CUSTOMERS[0].postalCode
    );
    ...
    await page.fill(
      '[name="creditLimit"], [data-testid="creditLimit"]',
      TEST_CUSTOMERS[0].creditLimit.toString()
    );
    await page.fill(
      '[name="creditDays"], [data-testid="creditDays"]',
      TEST_CUSTOMERS[0].creditDays.toString()
    );
    ...
    await expect(page.locator('text=บันทึกสำเร็จ, text=สำเร็จ, text=Success').first()).toBeVisible({
      timeout: 5000,
    });
    ...
    await expect(
      page
        .locator(`text=${TEST_CUSTOMERS[0].code}`)
        .or(page.locator(`text=${TEST_CUSTOMERS[0].name}`))
    ).toBeVisible({ timeout: 5000 });

    await page.screenshot({
      path: 'test-results/evidence/02-customer-create-1.png',
      fullPage: true,
    });
  ...
  test('[READ] View customer details', async ({ page }) => {
    ...
    await expect(
      page.locator('[role="dialog"], .modal, dialog').or(page.locator('text=รายละเอียดลูกค้า'))
    ).toBeVisible({ timeout: 5000 });
  ...
  test('[UPDATE] Edit customer information', async ({ page }) => {
    ...
    await page.click(
      'button:has-text("แก้ไข"), button[aria-label="Edit"], .edit-button, button:has-text("Edit")'
    );
  ...
  test('[DELETE] Cannot delete customer with transactions (if applicable)', async ({ page }) => {
    ...
    const deleteButton = page
      .locator('button:has-text("ลบ"), button:has-text("Delete"), .delete-button')
      .first();
    ...
    if (await deleteButton.isVisible({ timeout: 3000 })) {
      ...
      await expect(
        page.locator('[role="dialog"], .modal').or(page.locator('text=ยืนยัน'))
...
test.describe('Phase 2: Master Data Creation - Vendors (เจ้าหนี้)', () => {
  ...
  test('[CREATE] Create first vendor with complete data', async ({ page }) => {
    ...
    await page.click(
      'button:has-text("เพิ่มผู้ขาย"), button:has-text("สร้างใหม่"), button:has-text("สร้าง")'
    );
    ...
    await page.fill(
      '[name="creditDays"], [data-testid="creditDays"]',
      TEST_VENDORS[0].creditDays.toString()
    );

    await page.click('button:has-text("บันทึก"), button:has-text("保存")');

    await expect(
      page.locator(`text=${TEST_VENDORS[0].code}`).or(page.locator(`text=${TEST_VENDORS[0].name}`))
    ).toBeVisible({ timeout: 5000 });
...
test.describe('Phase 2: Master Data Creation - Products (สินค้า)', () => {
  ...
  test('[CREATE] Create product with inventory tracking', async ({ page }) => {
    ...
    await page.fill(
      '[name="salePrice"], [data-testid="salePrice"]',
      TEST_PRODUCTS[0].salePrice.toString()
    );
    await page.fill(
      '[name="costPrice"], [data-testid="costPrice"]',
      TEST_PRODUCTS[0].costPrice.toString()
    );
    ...
    await page.fill(
      '[name="quantity"], [data-testid="quantity"]',
      TEST_PRODUCTS[0].quantity.toString()
    );
    ...
    await expect(
      page
        .locator(`text=${TEST_PRODUCTS[0].code}`)
        .or(page.locator(`text=${TEST_PRODUCTS[0].name}`))
    ).toBeVisible({ timeout: 5000 });

    await page.screenshot({
      path: 'test-results/evidence/02-product-create-1.png',
      fullPage: true,
    });
  ...
  test('[CREATE] Create service product', async ({ page }) => {
    ...
    await page.fill(
      '[name="salePrice"], [data-testid="salePrice"]',
      TEST_PRODUCTS[1].salePrice.toString()
    );
  ...
  test('[CREATE] Create product with inclusive VAT', async ({ page }) => {
    ...
    await page.fill(
      '[name="salePrice"], [data-testid="salePrice"]',
      TEST_PRODUCTS[2].salePrice.toString()
    );
    await page.fill(
      '[name="costPrice"], [data-testid="costPrice"]',
      TEST_PRODUCTS[2].costPrice.toString()
    );
