...
async function hasChunkError(page: Page): Promise<boolean> {
  const bodyText = await page
    ...
    .catch(() => '');
  return (
    bodyText?.includes('Failed to load chunk') || bodyText?.includes('เกิดข้อผิดพลาด') || false
  );
}
...
async function loginAsAdmin(page: Page, maxRetries = 3): Promise<void> {
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      console.log(
        `  ❌ Login attempt ${attempt} failed: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
...
test.describe('AGENT_FULL_TEST - Full Coverage Test Suite', () => {
  ...
  for (const module of MODULES) {
    test(`[MODULE-${module.id.toUpperCase()}] ${module.name} - ${module.description}`, async ({
      page,
    }) => {
      console.log(`\n${'='.repeat(60)}`);
  ...
  test('[REPORT] Generate full coverage test report', async ({ page }) => {
    ...
    const totalErrors = testResults.reduce(
...
function generateMarkdownReport(
  results: TestResult[],
  consoleErrors: typeof consoleErrorLog
): string {
  ...
  reportLines.push('');
  reportLines.push(
    '| # | Module | Status | Load Time | Console Errors | Page Errors | Screenshot |'
  );
  reportLines.push(
    '|---|--------|--------|-----------|----------------|-------------|------------|'
  );
  ...
  results.forEach((result, index) => {
    ...
    const screenshotLink = result.screenshotPath
      ...
      : 'N/A';
    reportLines.push(
      `| ${index + 1} | ${result.moduleName} (${result.moduleId}) | ${status} | ${result.loadTime}ms | ${result.consoleErrors.length} | ${result.pageErrors.length} | ${screenshotLink} |`
    );
  });
  ...
  if (failed.length > 0) {
    ...
    for (const result of failed) {
      ...
      reportLines.push(
        `- **Sidebar Active:** ${result.verificationResults.sidebarActive ? 'Yes' : 'No'}`
      );
      reportLines.push(
        `- **Content Loaded:** ${result.verificationResults.contentLoaded ? 'Yes' : 'No'}`
      );
      reportLines.push(
        `- **Elements Found:** ${result.verificationResults.elementsFound.join(', ') || 'None'}`
      );
      reportLines.push(
        `- **Elements Missing:** ${result.verificationResults.elementsMissing.join(', ') || 'None'}`
      );
      ...
      if (result.consoleErrors.length > 0) {
        ...
        result.consoleErrors.forEach((err, i) => {
          reportLines.push(
            `  ${i + 1}. \`${err.substring(0, 100)}${err.length > 100 ? '...' : ''}\``
          );
        });
      }
  ...
  if (consoleErrors.length > 0) {
    ...
    for (const [module, errors] of Object.entries(errorsByModule)) {
      ...
      errors.forEach((err, i) => {
        reportLines.push(
          `${i + 1}. **${err.type.toUpperCase()}:** \`${err.message.substring(0, 150)}${err.message.length > 150 ? '...' : ''}\``
        );
      });
      reportLines.push('');
    }
  ...
  for (const module of MODULES) {
    reportLines.push(
      `| ${module.id} | ${module.name} | ${module.description} | ${module.expectedElements.join(', ')} |`
    );
  }
