...
test.describe('AGENT_SALES - Sales & Receivables Module', () => {
  ...
  test.describe('Invoices Module (ใบกำกับภาษี)', () => {
    ...
    test('[VERIFY] Header section elements', async ({ page }) => {
      ...
      const title = page.locator(
        'h1:has-text("ใบกำกับภาษี"), h1:has-text("เอกสารการขาย"), header h1, [class*="title"]'
      );
      ...
      const createBtn = page
        .locator(
          'button:has-text("สร้างเอกสารใหม่"), button:has-text("สร้าง"), [data-testid*="create"], button svg[class*="plus"]'
        )
        .first();
    ...
    test('[VERIFY] Summary cards (4 cards)', async ({ page }) => {
      ...
      const cards = page.locator(
        '[class*="card"], .stat-card, [class*="summary"], [class*="stats"] > div, [class*="grid"] > div'
      );
      const cardCount = await cards.count();
      console.log(`  Found ${cardCount} potential card elements`);

      // Check for specific card texts
      const cardTexts = ['รอออกใบกำกับภาษี', 'รอรับชำระ', 'รับชำระแล้ว', 'ภาษีขายรวม'];
    ...
    test('[VERIFY] Search and Filter section', async ({ page }) => {
      ...
      const searchInput = page
        .locator('input[placeholder*="ค้นหา"], input[type="search"], input[name*="search"]')
        .first();
      ...
      if (searchVisible) {
        ...
        const hasExpectedText =
          placeholder?.includes('ชื่อลูกค้า') ||
      ...
      const statusFilter = page
        .locator(
          'select, [role="combobox"], button:has-text("สถานะ"), button:has-text("ทั้งหมด"), [class*="filter"]'
        )
        .first();
      ...
      if (filterVisible) {
        ...
        for (const option of statusOptions) {
          const optionElement = page
            .locator(`text=${option}, [role="option"]:has-text("${option}")`)
            .first();
    ...
    test('[VERIFY] Invoice table columns', async ({ page }) => {
      ...
      for (const header of expectedHeaders) {
        const headerElement = table
          .locator(`th:has-text("${header}"), td:has-text("${header}")`)
          .first();
    ...
    test('[VERIFY] Action buttons per row', async ({ page }) => {
      ...
      if (rowCount > 0) {
        ...
        const actionButtons = firstRow.locator(
          'button, [role="button"], a[class*="button"], svg[class*="icon"]'
        );
        ...
        const viewBtn = firstRow
          .locator(
            'button[aria-label*="ดู"], button[title*="ดู"], button svg[class*="eye"], button svg[class*="view"]'
          )
          .first();
        const editBtn = firstRow
          ...
          .first();
        const printBtn = firstRow
          .locator(
            'button[aria-label*="พิมพ์"], button[title*="พิมพ์"], button svg[class*="print"], button svg[class*="printer"]'
          )
          .first();
        const downloadBtn = firstRow
          .locator(
            'button[aria-label*="ดาวน์โหลด"], button[title*="ดาวน์โหลด"], button svg[class*="download"]'
          )
          .first();
        ...
        expect(
          viewVisible || editVisible || printVisible || downloadVisible,
          'Should have at least one action button per row'
        ).toBeTruthy();
      } else {
    ...
    test('[VERIFY] Create Document Dialog', async ({ page }) => {
      ...
      const createBtn = page
        .locator(
          'button:has-text("สร้างเอกสารใหม่"), button:has-text("สร้าง"), [data-testid*="create"]'
        )
        .first();
      ...
      const dialog = page
        .locator('[role="dialog"], [class*="dialog"], [class*="modal"], [class*="popover"]')
        .first();
      ...
      if (dialogVisible) {
        ...
        for (const docType of docTypes) {
          const btn = dialog
            .locator(`button:has-text("${docType.name}"), text=${docType.name}`)
            .first();
        ...
        expect(
          foundDocTypes,
          'Should have at least 2 document type options'
  ...
  test.describe('Customers/AR Module (ลูกหนี้)', () => {
    ...
    test('[VERIFY] Customer list table', async ({ page }) => {
      ...
      const listContainer = page
        .locator('[class*="list"], [class*="table"], [class*="grid"]')
        .first();
      ...
      for (const header of customerHeaders) {
        const headerElement = page
          .locator(`th:has-text("${header}"), td:has-text("${header}"), text=${header}`)
          .first();
        if (await headerElement.isVisible().catch(() => false)) {
          foundHeaders++;
      ...
      expect(
        foundHeaders,
        'Should have at least 2 customer-related columns'
      ).toBeGreaterThanOrEqual(2);
    });
    ...
    test('[VERIFY] Add Customer button', async ({ page }) => {
      ...
      const addBtn = page
        .locator(
          'button:has-text("เพิ่มลูกค้า"), button:has-text("สร้างลูกค้า"), button:has-text("เพิ่มใหม่"), button svg[class*="plus"]'
        )
        .first();
    ...
    test('[VERIFY] Delete Customer button per row', async ({ page }) => {
      ...
      if (rowCount > 0) {
        ...
        const deleteBtn = firstRow
          .locator(
            'button[aria-label*="ลบ"], button[title*="ลบ"], button svg[class*="delete"], button svg[class*="trash"]'
          )
          .first();
        ...
        if (deleteVisible) {
          ...
          const confirmDialog = page
            .locator(
              '[role="dialog"]:has-text("ลบ"), [role="dialog"]:has-text("ยืนยัน"), [class*="confirm"]'
            )
            .first();
    ...
    test('[VERIFY] AR Aging view', async ({ page }) => {
      ...
      for (const element of agingElements) {
        const el = page
          .locator(`text=${element}, button:has-text("${element}"), [class*="aging"]`)
          .first();
      ...
      for (const tab of await tabs) {
        ...
        if (
          tabText &&
          (tabText.includes('อายุ') || tabText.includes('Aging') || tabText.includes('ค้างชำระ'))
        ) {
      ...
      expect(
        foundAgingElements,
        'Should have at least 1 AR Aging related element'
  ...
  test('[SUMMARY] All Sales & AR module buttons', async ({ page }) => {
    ...
    const invoicesChecks = [
      {
        name: 'สร้างเอกสารใหม่ (Create New Document)',
        selector: 'button:has-text("สร้าง"), [data-testid*="create"]',
      },
      { name: 'Search input', selector: 'input[placeholder*="ค้นหา"]' },
      { name: 'Status filter', selector: 'select, [role="combobox"], button:has-text("สถานะ")' },
      {
        name: 'View button (per row)',
        selector: 'button svg[class*="eye"], button[aria-label*="ดู"]',
      },
      {
        name: 'Edit button (per row)',
        selector: 'button svg[class*="edit"], button[aria-label*="แก้ไข"]',
      },
      {
        name: 'Print button (per row)',
        selector: 'button svg[class*="print"], button[aria-label*="พิมพ์"]',
      },
      {
        name: 'Download button (per row)',
        selector: 'button svg[class*="download"], button[aria-label*="ดาวน์โหลด"]',
      },
    ];
    ...
    if (await createBtn.isVisible().catch(() => false)) {
      ...
      for (const check of dialogChecks) {
        const element = page
          .locator(`button:has-text("${check.text}"), text=${check.text}`)
          .first();
        const visible = await element.isVisible().catch(() => false);
        summary.invoices.buttons.push(`${visible ? '✅' : '❌'} ${check.name}`);
      }
    ...
    const customersChecks = [
      {
        name: 'เพิ่มลูกค้า (Add Customer)',
        selector: 'button:has-text("เพิ่ม"), button svg[class*="plus"]',
      },
      { name: 'Search input', selector: 'input[placeholder*="ค้นหา"]' },
      {
        name: 'Edit button (per row)',
      ...
      {
        name: 'Delete button (per row)',
        selector: 'button svg[class*="trash"], button[aria-label*="ลบ"]',
      },
    ];
