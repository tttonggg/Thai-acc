...
test.describe('AGENT_ADMIN - Administration Modules (ADMIN only)', () => {
  ...
  test('[ADMIN-007] User Management module - Delete User Dialog', async ({ page }) => {
    ...
    try {
      ...
      const cancelButton = page
        .locator(
          'div[role="dialog"] button:has-text("ยกเลิก"), div[role="alertdialog"] button:has-text("ยกเลิก")'
        )
        .first();
      await expect(cancelButton).toBeVisible();
      console.log('✓ Cancel button is visible');
      ...
      const confirmDeleteButton = page
        .locator(
          'div[role="dialog"] button:has-text("ลบ"), div[role="alertdialog"] button:has-text("ลบ")'
        )
        .filter({ hasText: /^ลบ$/ })
        .first();
  ...
  test('[ADMIN-009] Settings module - Save Company Info', async ({ page }) => {
    ...
    try {
      ...
      await page.fill(
        'label:has-text("ชื่อบริษัท (ไทย)") + div input, label:has-text("ชื่อบริษัท (ไทย)") >> xpath=../following-sibling::input',
        'Test Company ทดสอบ'
      );
      ...
      // Fill in tax ID
      await page.fill(
        'label:has-text("เลขประจำตัวผู้เสียภาษี") + div input, label:has-text("เลขประจำตัวผู้เสียภาษี") >> xpath=../following-sibling::input',
        '1234567890123'
      );
