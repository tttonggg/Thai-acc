...
test.describe('AGENT_VALIDATOR - UI-DB Alignment Verification', () => {
   ...
   */
  test('[ALIGN-001] Chart of Accounts: UI count matches DB (181 accounts)', async ({
    page,
    request,
  }) => {
    console.log('\n==========================================');
  ...
  test('[ALIGN-002] Users: UI matches DB records', async ({ page, request }) => {
    ...
    try {
      ...
      const roleBadges = page.locator(
        'table tbody tr td:has-text("ผู้ดูแลระบบ"), table tbody tr td:has-text("นักบัญชี"), table tbody tr td:has-text("ผู้ใช้ทั่วไป"), table tbody tr td:has-text("ผู้ดูเท่านั้น")'
      );
    ...
    try {
      ...
      for (const verification of tableVerifications) {
        try {
          ...
          console.log(
            `${icon} Table ${verification.table} (${verification.name}): ${count} records`
          );
        } catch (error) {
          results.push({
            table: verification.table,
      ...
      console.log('\n------------------------------------------');
      console.log(
        `Schema Verification Summary: ${existingTables}/${totalTables} required tables verified`
      );
      console.log('------------------------------------------');
  ...
  test('[ALIGN-005] Chart of Accounts structure verification', async ({ page, request }) => {
    ...
    try {
      ...
      accounts.forEach((account) => {
        ...
        switch (firstDigit) {
          case '1':
            structure.byCodeRange['1xxx (Assets)']++;
            break;
          case '2':
            structure.byCodeRange['2xxx (Liabilities)']++;
            break;
          case '3':
            structure.byCodeRange['3xxx (Equity)']++;
            break;
          case '4':
            structure.byCodeRange['4xxx (Revenue)']++;
            break;
          case '5':
            structure.byCodeRange['5xxx (Expenses)']++;
            break;
          default:
            structure.byCodeRange['Other']++;
        }
      });
