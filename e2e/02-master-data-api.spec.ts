...
test.describe('Phase 2: Master Data Creation via API', () => {
  ...
  test('[CREATE] Create first customer', async ({ }) => {
    ...
    if (existing) {
      console.log(`ℹ️ [CREATE] Customer ${TEST_CUSTOMERS[0].code} already exists, skipping creation`)
  ...
  test('[CREATE] Create second customer', async ({ }) => {
    ...
    if (existing) {
      console.log(`ℹ️ [CREATE] Customer ${TEST_CUSTOMERS[1].code} already exists, skipping creation`)
