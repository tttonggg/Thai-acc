...
async function login(page: Page, email: string, password: string) {
  ...
  await expect(page.locator('nav, aside, [data-testid="sidebar"], h1').first()).toBeVisible({
    timeout: 30000,
  });
}
...
test.describe('Authentication Testing', () => {
  ...
  test('should login successfully with valid credentials', async ({ page }) => {
    ...
    await expect(
      page
        .locator('text=รายได้รวม')
        .or(page.locator('text=ค่าใช้จ่ายรวม'))
        .or(page.locator('nav'))
        .first()
  ...
  test('should persist session across page reloads', async ({ page }) => {
    ...
    await expect(
      page.locator('h1:has-text("ภาพรวมธุรกิจ")').or(page.locator('nav')).first()
  ...
  test('auth should work consistently without intermittent failures', async ({ page }) => {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      const logoutButton = page
        .locator('button:has-text("ออกจากระบบ"), button[aria-label="Logout"]')
        .first();
...
test.describe('Invoice Edit Dialog - Tabs', () => {
  ...
  test('should open edit dialog with tabs', async ({ page }) => {
    ...
    const editButton = firstRow
      .locator('button')
      .filter({ has: page.locator('svg[data-lucide="edit"], .lucide-edit') })
      .first();
  ...
  test('should switch between Details and Comments tabs', async ({ page }) => {
    ...
    await expect(
      page
        .locator('text=ความคิดเห็น')
        .or(page.locator('.comment-section'))
    ...
    await expect(
      page.locator('text=เลขที่เอกสาร').or(page.locator('label:has-text("ลูกค้า")')).first()
    ).toBeVisible();
  });
...
test.describe('Comment Operations', () => {
  ...
  test('should add a test comment', async ({ page }) => {
    ...
    const commentInput = page
      .locator(
        'textarea[placeholder*="ความคิดเห็น"], textarea[placeholder*="comment"], [data-testid="comment-input"]'
      )
      .first();
    ...
    if (await commentInput.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const submitButton = page
        .locator(
          'button:has-text("ส่ง"), button:has-text("โพสต์"), button:has-text("Post"), button[type="submit"]'
        )
      ...
      if (await submitButton.isVisible({ timeout: 3000 }).catch(() => false)) {
        ...
        await expect(page.locator(`text=${testComment.substring(0, 30)}`)).toBeVisible({
          timeout: 5000,
        });
      }
    } else {
  ...
  test('should reply to a comment', async ({ page }) => {
    ...
    const replyButton = page
      .locator('button:has-text("ตอบกลับ"), button:has-text("Reply"), [data-testid="reply-button"]')
      .first();
    ...
    if (await replyButton.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      if (await replyInput.isVisible({ timeout: 3000 }).catch(() => false)) {
        ...
        const submitReply = page
          .locator(
            'button:has-text("ส่ง"), button:has-text("โพสต์"), button:has-text("Post"), button:has-text("Reply"), button:has-text("ตอบกลับ")'
          )
          .first();
        ...
        await expect(page.locator(`text=${replyText.substring(0, 20)}`)).toBeVisible({
          timeout: 5000,
        });
      }
    } else {
  ...
  test('should resolve and unresolve a comment', async ({ page }) => {
    ...
    const resolveButton = page
      .locator(
        'button:has-text("แก้ไขแล้ว"), button:has-text("Resolve"), button:has-text("ตกลง"), [data-testid="resolve-button"]'
      )
      .first();
    ...
    if (await resolveButton.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const unresolveButton = page
        .locator(
          'button:has-text("ยกเลิกการแก้ไข"), button:has-text("Unresolve"), button:has-text("ยังไม่แก้ไข"), [data-testid="unresolve-button"]'
        )
        .first();
  ...
  test('should update comment count after adding comment', async ({ page }) => {
    ...
    const firstRow = page.locator('tbody tr').first();
    const initialBadge = firstRow
      .locator('td:nth-child(9) .badge, td:nth-child(9) [class*="badge"]')
      .first();
  ...
  test('comments section should have proper styling', async ({ page }) => {
    ...
    if (await commentSection.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const emptyState = page
        .locator('text=ยังไม่มีความคิดเห็น, text=No comments, .empty-state')
        .first();
...
test.describe('Error Handling', () => {
  ...
  test('should handle network errors gracefully', async ({ page }) => {
    ...
    await expect(
      page.locator('text=ข้อผิดพลาด').or(page.locator('text=error')).first()
    ).toBeVisible({ timeout: 10000 });
  ...
  test('should handle unauthorized access', async ({ page }) => {
    ...
    const errorVisible = await page
      .locator('text=ไม่ได้รับอนุญาต, text=unauthorized, text=เข้าสู่ระบบ')
      .first()
      .isVisible({ timeout: 5000 })
      .catch(() => false);
