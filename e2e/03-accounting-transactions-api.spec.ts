...
test.describe('Phase 3: GL Posting Verification', () => {
  ...
  test('[VERIFY] Get journal entries and verify double-entry', async ({}) => {
    ...
    for (const entry of entries.slice(0, 5)) {
      const totalDebit =
        entry.lines?.reduce((sum: number, line: any) => sum + (line.debit || 0), 0) || 0;
      const totalCredit =
        entry.lines?.reduce((sum: number, line: any) => sum + (line.credit || 0), 0) || 0;
      ...
      if (isBalanced) {
        balancedCount++;
        console.log(
          `  ✅ Entry ${entry.journalNo}: Debit=${totalDebit}, Credit=${totalCredit} (BALANCED)`
        );
      } else {
        unbalancedCount++;
        console.log(
          `  ❌ Entry ${entry.journalNo}: Debit=${totalDebit}, Credit=${totalCredit} (UNBALANCED)`
        );
      }
    }
