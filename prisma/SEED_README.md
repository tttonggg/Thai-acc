...
The seed function was failing with
`Unique constraint failed on the fields: (invoiceNo)` when run multiple times
because it was trying to create duplicate invoice numbers.
...
4. **Constraint Error**: Catches P2002 errors, fetches existing record,
   continues
...
The warning about MODULE_TYPELESS_PACKAGE_JSON is informational and doesn't
affect functionality. It can be safely ignored or resolved by adding
`"type": "module"` to package.json if needed.
