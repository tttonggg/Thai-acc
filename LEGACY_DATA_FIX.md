...
Instead of always converting (which breaks old invoices) or never converting (which breaks new invoices), we **detect the format**:
...
**Status:** ✅ Fixed with format detection (temporary solution)
**Next Step:** Data migration to convert all old invoices to Satang format
