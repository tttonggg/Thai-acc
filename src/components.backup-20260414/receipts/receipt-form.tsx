                      ...
                      {unpaidInvoices.map((invoice: any) => {
                        ...
                        return (
                          <TableRow key={invoice.id}>
                            ...
                            <TableCell className="text-right">฿{(invoice.totalAmount / 100).toLocaleString()}</TableCell>
                            <TableCell className="text-right">฿{(invoice.balance / 100).toLocaleString()}</TableCell>
                            <TableCell className="text-right">
                              {allocation ? (
                                <Input className="!h-11 text-base"
                                  type="number"
                                  step="0.01"
                                  value={allocation.amount / 100}
                            ...
                            <TableCell className="text-right">
                              {allocation ? `฿${(allocation.whtAmount / 100).toLocaleString()}` : '-'}
                            </TableCell>
                            <TableCell>
                              {allocation && (
                                <Button
              ...
              <CardContent className="p-6">
                <div className="space-y-2">
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold">฿{(amount / 100).toLocaleString()}</span>
                  </div>
                  {totalAllocated > 0 && (
                    <>
                      <div className="flex justify-between">
                        <span>จัดจ่ายใบกำกับภาษี</span>
                        <span>฿{(totalAllocated / 100).toLocaleString()}</span>
                      </div>
                      {totalWht > 0 && (
                        <div className="flex justify-between">
                          <span>ภาษีหัก ณ ที่จ่าย</span>
                          <span>฿{(totalWht / 100).toLocaleString()}</span>
                        </div>
                      )}
                    </>
                  )}
