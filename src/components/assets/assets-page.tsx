...
function AssetListTab() {
  ...
  const handleSubmit = async () => {
    ...
    if (res.success) {
      toast({
        title: 'บันทึกสำเร็จ',
        description: `สินทรัพย์ ${form.name} ถูกสร้างพร้อมตารางค่าเสื่อมราคาแล้ว`,
      });
      setShowAdd(false);
      setForm({
        code: '',
  ...
  const handleEditAsset = async (asset: Asset) => {
    ...
    try {
      ...
      toast({
        title: 'ข้อผิดพลาด',
        description: 'ไม่สามารถดึงข้อมูลสินทรัพย์ได้',
        variant: 'destructive',
      });
    }
  };
  ...
  const confirmDeleteAsset = async () => {
    ...
    try {
      const res = await window
        .fetch(`/api/assets/${assetToDelete.id}`, { credentials: 'include', method: 'DELETE' })
        .then((r) => r.json());

      if (res.success) {
        toast({
          title: 'ลบสำเร็จ',
          description: `ลบสินทรัพย์ ${assetToDelete.name} เรียบร้อยแล้ว`,
        });
      ...
      toast({
        title: 'ข้อผิดพลาด',
        description: 'ไม่สามารถลบสินทรัพย์ได้',
        variant: 'destructive',
      });
    }
  };
  ...
  const handleViewSchedule = async (asset: Asset) => {
    ...
    try {
      ...
      toast({
        title: 'ข้อผิดพลาด',
        description: 'ไม่สามารถดึงตารางค่าเสื่อมราคาได้',
        variant: 'destructive',
      });
    }
  };
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-3 gap-4">
        <Card className="border-l-4 border-l-blue-500">
          <CardContent className="p-4">
            <p className="text-xs text-gray-500">จำนวนสินทรัพย์</p>
            <p className="text-2xl font-bold text-blue-600">{assets.length} รายการ</p>
          </CardContent>
        </Card>
        <Card className="border-l-4 border-l-orange-500">
          <CardContent className="p-4">
            <p className="text-xs text-gray-500">ราคาทุนรวม</p>
            <p className="text-2xl font-bold text-orange-600">฿{fc(totalCost)}</p>
          </CardContent>
        </Card>
        <Card className="border-l-4 border-l-green-500">
          <CardContent className="p-4">
            <p className="text-xs text-gray-500">มูลค่าสุทธิรวม (NBV)</p>
            <p className="text-2xl font-bold text-green-600">฿{fc(totalNBV)}</p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="flex justify-end">
        <Dialog open={showAdd} onOpenChange={setShowAdd}>
          ...
          <DialogContent className="max-w-lg">
            <DialogHeader>
              <DialogTitle>ลงทะเบียนสินทรัพย์ถาวร (TAS 16)</DialogTitle>
            </DialogHeader>
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setForm((p) => ({ ...p, salvageValue: e.target.value }))}
                  />
                </div>
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setForm((p) => ({ ...p, usefulLifeYears: e.target.value }))}
                  />
                </div>
              </div>
              <div className="rounded bg-gray-50 p-2 text-xs text-gray-400">
                ...
                {form.purchaseCost && form.salvageValue && form.usefulLifeYears
                  ? fc(
                      (parseFloat(form.purchaseCost) - parseFloat(form.salvageValue)) /
                        (parseInt(form.usefulLifeYears) * 12)
                    )
                  : '—'}{' '}
                ฿/เดือน
              </div>
              <div>
                <Label>บัญชีสินทรัพย์ (GL ID)</Label>
                <Input
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setForm((p) => ({ ...p, accumDepAccountId: e.target.value }))}
                  />
                </div>
                <div>
                  <Label>บัญชีค่าเสื่อมประจำปี</Label>
                  <Input
                    value={form.depExpenseAccountId}
                    onChange={(e) =>
                      setForm((p) => ({ ...p, depExpenseAccountId: e.target.value }))
                    }
                  />
                </div>
              </div>
                  ...
                  assets.map((a) => (
                    <TableRow key={a.id}>
                      ...
                      <TableCell className="text-right font-semibold text-green-600">
                        ฿{fc(a.netBookValue ?? a.purchaseCost)}
                      </TableCell>
      ...
      <Dialog open={showDeleteConfirm} onOpenChange={setShowDeleteConfirm}>
        <DialogContent>
          ...
          <div className="py-4">
            ...
            <p className="mt-2 text-sm text-gray-500">การดำเนินการนี้ไม่สามารถย้อนกลับได้</p>
          </div>
          <DialogFooter>
    ...
    <div className="space-y-0">
      <div className="mb-4">
        ...
        <p className="text-sm text-gray-500">
          ทะเบียนสินทรัพย์ถาวรและตารางค่าเสื่อมราคา — TAS 16 (เส้นตรง)
        </p>
      </div>
      <AssetListTab />
    </div>
