...
## Purpose
Global layout shell — sidebar navigation, header, and module routing. SPA pattern using `activeModule` state (not standard Next.js file routing).
...
// Module types
type Module =
  | 'dashboard' | 'invoices' | 'receipts' | 'payments'
  | 'purchases' | 'inventory' | 'assets' | 'banking'
  | 'petty_cash' | 'payroll' | 'wht' | 'quotations'
  | 'journal' | 'reports' | 'settings' | 'admin'
```
