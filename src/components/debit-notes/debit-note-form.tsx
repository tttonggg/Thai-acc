...
export function DebitNoteForm({ open, onClose, onSuccess }: DebitNoteFormProps) {
  ...
  const form = useForm<z.infer<typeof debitNoteSchema>>({
    ...
    defaultValues: {
      ...
      lines: [
        { description: '', quantity: 1, unit: 'ชิ้น', unitPrice: 0, discount: 0, vatRate: 7 },
      ],
    },
  });
  ...
  useEffect(() => {
    const fetchPurchaseInvoices = async () => {
      ...
      try {
        ...
        setPurchaseInvoices(
          purchaseData.filter((inv: PurchaseInvoice) => inv.status !== 'CANCELLED')
        );
      } catch (error) {
  ...
  const updateLine = (index: number, field: string, value: any) => {
    ...
    form.setValue(
      'lines',
      currentLines.map((line, i) => (i === index ? { ...line, [field]: value } : line))
    );
  };
  ...
  const totals = lines.reduce(
    (acc, line) => {
      ...
      return {
        ...
        totalAmount: acc.totalAmount + amount + vat,
      };
    },
  ...
  const onSubmit = async (values: z.infer<typeof debitNoteSchema>) => {
    ...
    try {
      ...
      toast({
        title: 'สำเร็จ',
        description: `สร้างใบเพิ่มหนี้ ${data.data.debitNoteNo} เรียบร้อยแล้ว`,
      });
      form.reset();
      onSuccess();
    } catch (error) {
      toast({
        ...
        description: error instanceof Error ? error.message : 'สร้างใบเพิ่มหนี้ไม่สำเร็จ',
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-h-[90vh] max-w-[95vw] overflow-y-auto md:max-w-4xl">
        ...
        >
            ...
            <CardContent className="space-y-4">
                  ...
                  <Popover>
                    <PopoverTrigger asChild>
                      <Button variant="outline" className="w-full justify-start">
                        ...
                        {form.watch('debitNoteDate') ? (
                          new Date(form.watch('debitNoteDate')).toLocaleDateString('th-TH')
                        ) : (
                    ...
                    <PopoverContent>
                      <Calendar
                        mode="single"
                        selected={new Date(form.watch('debitNoteDate'))}
                        onSelect={(date) =>
                          date && form.setValue('debitNoteDate', date.toISOString())
                        }
                      />
                    </PopoverContent>
                  </Popover>
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                <div className="space-y-2">
                  ...
                  <Select
                    value={form.watch('purchaseInvoiceId') || ''}
                    onValueChange={(v) => form.setValue('purchaseInvoiceId', v || null)}
                  >
                    <SelectTrigger className="!h-11 text-base">
                      <SelectValue placeholder="เลือกใบซื้อ" />
                    </SelectTrigger>
                    <SelectContent>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-lg">รายการที่เพิ่มหนี้</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
                      ...
                      <Select
                        ...
                        onValueChange={(v) => {
                          ...
                          if (p) {
                            ...
                            updateLine(index, 'unitPrice', p.costPrice);
                            updateLine(index, 'vatRate', p.vatRate);
                          }
                        }}
                      >
                        <SelectTrigger className="!h-11 text-base">
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              ...
              <div className="flex justify-between text-lg font-bold">
                <span>ยอดรวม:</span>
                <span className="text-orange-600">+฿{totals.totalAmount.toLocaleString()}</span>
              </div>
            </CardContent>
          </Card>
          ...
          <Alert>
            <AlertDescription>
              การออกใบเพิ่มหนี้จะเพิ่มหนี้ผู้ขายและบันทึกบัญชีอัตโนมัติ
            </AlertDescription>
          </Alert>
          ...
          <div className="flex justify-end gap-3">
            ...
            <Button type="submit" className="bg-orange-600 hover:bg-orange-700" disabled={loading}>
              {loading ? 'กำลังบันทึก...' : 'ออกใบเพิ่มหนี้'}
            </Button>
          </div>
        </form>
      </DialogContent>
