...
export function CreditNoteForm({ open, onClose, onSuccess, creditNoteId }: CreditNoteFormProps) {
    ...
    const fetchInvoices = async () => {
      ...
      try {
        const res = await fetch(
          `/api/invoices?customerId=${selectedCustomerId}&status=ISSUED,PAID`,
          { credentials: 'include' }
        );
        const data = await res.json();
  ...
  const updateLine = (index: number, field: string, value: any) => {
    ...
    form.setValue(
      'lines',
      currentLines.map((line, i) => (i === index ? updatedLine : line))
    );
  };
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    aria-describedby={
                      form.formState.errors.customerId ? 'customerId-error' : undefined
                    }
                  >
                  ...
                  {form.formState.errors.customerId && (
                    <p id="customerId-error" className="text-sm text-red-500" role="alert">
                ...
                <div className="space-y-2">
                  ...
                  <Popover>
                    ...
                    <PopoverContent className="w-auto p-0" align="start">
                      <Calendar
                        ...
                        onSelect={(date) =>
                          date && form.setValue('creditNoteDate', date.toISOString())
                        }
                        initialFocus
                      />
                    </PopoverContent>
                  ...
                  {form.formState.errors.creditNoteDate && (
                    <p id="creditNoteDate-error" className="text-sm text-red-500" role="alert">
                      {form.formState.errors.creditNoteDate.message}
                    </p>
                  )}
                </div>
              </div>
              ...
              {lines.map((line, index) => (
                <div key={index} className="space-y-3 rounded-lg border p-4">
                  <div className="flex items-start justify-between">
                    <h4 className="font-medium" id={`line-heading-${index}`}>
                      รายการที่ {index + 1}
                    </h4>
                    {lines.length > 1 && (
                      <Button
