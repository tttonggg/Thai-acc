            ...
            <TableBody>
              {currencies.map((currency) => (
                <TableRow key={currency.id}>
                  ...
                  <TableCell>
                    {currency.isBase && (
                      <Badge variant="default" className="bg-green-500">สกุลหลัก</Badge>
                    )}
                  </TableCell>
                </TableRow>
              ))}
          ...
          {report && (
            <div className="space-y-4">
              <div className="text-sm text-muted-foreground">
                สกุลเงินหลัก: {report.baseCurrency} | วันที่รายงาน: {new Date(report.reportDate).toLocaleDateString("th-TH")}
              </div>
              <Table>
                <TableHeader>
                  <TableRow>
                ...
                <TableBody>
                  {report.currencies?.map((c: any) => (
                    <TableRow key={c.code}>
                      ...
                      <TableCell className={c.realizedGainLoss + c.unrealizedGainLoss >= 0 ? "text-green-600" : "text-red-600"}>
