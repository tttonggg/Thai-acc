...
import { Wallet, Receipt, Plus, RefreshCw, Pencil, Trash2, CheckCircle, DollarSign, AlertCircle } from 'lucide-react'
...
interface Fund {
  ...
  custodianId: string; glAccountId: string
...
interface Voucher {
  ...
  fundId: string; glExpenseAccountId: string
    ...
    <div className="space-y-4">
      ...
      <div className="flex justify-end">
        <Button size="sm" onClick={handleCreate} className="bg-blue-600 hover:bg-blue-700">
          <Plus className="h-4 w-4 mr-1" />สร้างกองทุน
        </Button>
      </div>
              ...
              <CardContent className="p-5">
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-gray-500 mt-1">ผู้ถือ: {f.custodian?.name || '—'}</p>
                  </div>
        ...
        <DialogContent>
          ...
          <div className="py-4">
            <p className="text-gray-700">
              คุณต้องการลบกองทุน <span className="font-semibold">{deleteConfirm?.name}</span> ใช่หรือไม่?
            </p>
...
function VouchersTab({ funds }: { funds: Fund[] }) {
  ...
  const handleApprove = async (voucher: Voucher) => {
    try {
      ...
      toast({ title: 'ข้อผิดพลาด', description: 'เกิดข้อผิดพลาดในการอนุมัติ', variant: 'destructive' })
    }
  }

  const handleReimburse = async () => {
    if (!reimburseDialog || !cashBankAccountId) {
      toast({ title: 'ข้อผิดพลาด', description: 'กรุณาระบุบัญชีเงินสด/ธนาคาร', variant: 'destructive' })
      return
    }
    ...
    try {
      ...
      toast({ title: 'ข้อผิดพลาด', description: 'เกิดข้อผิดพลาดในการเติมเงิน', variant: 'destructive' })
    }
  }
    ...
    <div className="space-y-4">
          ...
          <Dialog open={showAdd} onOpenChange={setShowAdd}>
            <DialogTrigger asChild>
              <Button size="sm" className="bg-blue-600 hover:bg-blue-700">
                <Plus className="h-4 w-4 mr-1" />บันทึกใบสำคัญ
              </Button>
            </DialogTrigger>
            <DialogContent>
      ...
      <Dialog open={!!deleteConfirm} onOpenChange={() => setDeleteConfirm(null)}>
        <DialogContent>
          ...
          <div className="py-4">
            <p className="text-gray-700">
              คุณต้องการลบใบสำคัญ <span className="font-semibold">{deleteConfirm?.voucherNo}</span> ใช่หรือไม่?
            </p>
            <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
              <p className="text-sm text-yellow-800">
                <AlertCircle className="h-4 w-4 inline mr-1" />
      ...
      <Dialog open={!!reimburseDialog} onOpenChange={() => { setReimburseDialog(null); setCashBankAccountId('') }}>
        <DialogContent>
          <DialogHeader>
          ...
          <div className="py-4">
            <p className="text-gray-700 mb-4">
              เติมเงินกองทุนสำหรับใบสำคัญ <span className="font-semibold">{reimburseDialog?.voucherNo}</span>
            </p>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => { setReimburseDialog(null); setCashBankAccountId('') }}>
              ยกเลิก
            </Button>
