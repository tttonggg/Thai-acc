...
export function BankStatementImport() {
  ...
  const handleFiles = async (selectedFiles: File[]) => {
    const validFiles = selectedFiles.filter(
      (f) => f.name.endsWith('.xml') || f.name.endsWith('.camt053') || f.name.endsWith('.json')
    );
  ...
  const parseCamtXml = (xml: string) => {
    ...
    for (const block of ntryMatches) {
      ...
      const valDtMatch = block.match(
        /<ValDt>[\s\S]*?<Dt>(\d{4}-\d{2}-\d{2})<\/Dt>[\s\S]*?<\/ValDt>/
      );
    ...
    <div className="space-y-6">
      ...
      >
        <CardContent className="p-8 text-center">
          ...
          <p className="mb-1 text-sm text-gray-600">ลากไฟล์มาวางที่นี่ หรือคลิกเพื่อเลือก</p>
          <p className="text-xs text-gray-400">รองรับ .xml, .camt053, .json (CAMT.053 format)</p>
        </CardContent>
      </Card>
                ...
                <TableBody>
                  {parsedEntries.slice(0, 50).map((entry, i) => (
                    <TableRow key={i}>
                      ...
                      <TableCell className="max-w-xs truncate text-sm" title={entry.description}>
                        {entry.description}
                      </TableCell>
                      <TableCell>
                        <Badge
                      ...
                      <TableCell
                        className={`whitespace-nowrap text-right font-semibold ${entry.type === 'CREDIT' ? 'text-green-600' : 'text-red-600'}`}
                      >
