      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="flex items-center gap-2 text-base">
            ...
            ยอดคงเหลือสินค้า
            <Button size="sm" variant="ghost" onClick={fetch}>
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      const [movRes, whRes, prodRes] = await Promise.all([
        window
          .fetch(`/api/stock-movements${params}`, { credentials: 'include' })
          .then((r) => r.json()),
        window.fetch(`/api/warehouses`, { credentials: 'include' }).then((r) => r.json()),
  ...
  const handleViewMovement = async (movementId: string) => {
    try {
      const res = await window
        .fetch(`/api/stock-movements/${movementId}`, { credentials: 'include' })
        .then((r) => r.json());
      if (res.success) {
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex gap-2">
          {['ALL', 'RECEIVE', 'ISSUE', 'TRANSFER_IN', 'TRANSFER_OUT', 'ADJUST'].map((t) => (
            <button
              ...
              className={`rounded-full border px-3 py-1 text-xs transition-colors ${typeFilter === t ? 'border-blue-600 bg-blue-600 text-white' : 'border-gray-200 text-gray-600 hover:bg-gray-50'}`}
            >
        ...
        <Dialog open={showAdd} onOpenChange={setShowAdd}>
          ...
          <DialogContent>
            <DialogHeader>
              <DialogTitle>บันทึกการเคลื่อนไหวสินค้า</DialogTitle>
            </DialogHeader>
                  ...
                  movements.map((m) => {
                    ...
                    return (
                      <TableRow key={m.id}>
                        ...
                        <TableCell>
                          <span
                            className={`rounded-full px-2 py-0.5 text-xs font-medium ${mt.color}`}
                          >
                            {mt.label}
                          </span>
                        </TableCell>
                        ...
                        <TableCell
                          className={`text-right font-semibold ${isOut ? 'text-red-600' : 'text-green-600'}`}
                        >
                          {isOut ? '-' : '+'}
                          {fc(Math.abs(m.quantity))}
                        </TableCell>
...
function WarehousesTab() {
  ...
  const handleDelete = async (warehouse: WarehouseItem) => {
    ...
    try {
      const res = await window
        .fetch(`/api/warehouses/${warehouse.id}`, { credentials: 'include', method: 'DELETE' })
        .then((r) => r.json());
            ...
            <CardContent className="p-5">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  {w.location && <p className="mt-1 text-xs text-gray-500">📍 {w.location}</p>}
...
function StockTransfersTab() {
  ...
  const handleSubmit = async () => {
    if (formData.fromWarehouseId === formData.toWarehouseId) {
      toast({
        title: 'ข้อผิดพลาด',
        description: 'คลังต้นทางและคลังปลายทางต้องไม่ใช่คลังเดียวกัน',
        variant: 'destructive',
      });
      return;
    }
      ...
      <div className="flex justify-end">
        <Dialog open={showAdd} onOpenChange={setShowAdd}>
          ...
          <DialogContent className="max-w-md">
            <DialogHeader>
              <DialogTitle>โอนสินค้าระหว่างคลัง</DialogTitle>
            </DialogHeader>
            <div className="space-y-3">
              <div>
                <Label>จากคลัง *</Label>
                <Select
                  ...
                  onValueChange={(v) => setFormData((p) => ({ ...p, fromWarehouseId: v }))}
                >
                  <SelectTrigger>
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(v) => setFormData((p) => ({ ...p, toWarehouseId: v }))}
                >
                  <SelectTrigger>
              ...
              <div>
                ...
                <Input
                  value={formData.notes}
                  onChange={(e) => setFormData((p) => ({ ...p, notes: e.target.value }))}
                  placeholder="เหตุผลการโอน (ถ้ามี)"
                />
              </div>
            </div>
            <DialogFooter>
...
export function InventoryPage({
  initialTab,
}: { initialTab?: 'balance' | 'movements' | 'warehouses' | 'transfers' } = {}) {
  const [tab, setTab] = useState<'balance' | 'movements' | 'warehouses' | 'transfers'>(
    initialTab || 'balance'
  );
