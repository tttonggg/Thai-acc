...
## Purpose
Stock management — multi-warehouse inventory tracking, stock movements (IN/OUT/TRANSFER), stock takes (physical counts), adjustments, and unit cost calculation. TAS 2 compliant for Thai revenue department.
...
const calculateUnitCost = async (productId: string, warehouseId: string) => {
  ...
  const totalCost = movements.reduce((sum, m) => sum + (m.unitCost * m.quantity), 0)
