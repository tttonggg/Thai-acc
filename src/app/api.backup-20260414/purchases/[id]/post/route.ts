...
export async function POST(
  ...
  try {
    ...
    const lineProductIds = existing.lines.map(l => l.productId).filter((id): id is string => id !== null)
        ...
        if (purchaseOrder) {
          ...
          for (const invLine of existing.lines) {
            ...
            if (invLine.productId) {
              matchedPOLine = purchaseOrder.lines.find(l => l.productId === invLine.productId) ?? null
            }
            ...
            const qtyVariancePct = qtyGRN > 0 ? (qtyVariance / qtyGRN) * 100 : (qtyInvoice > 0 ? 999 : 0)
          ...
          if (blockedLines.length > 0) {
            const summary = blockedLines
              ...
              .join("; ")
            throw new Error(
              `ไม่สามารถรับใบซื้อ — พบความแตกต่างเกินขีดจำกัด: ${summary}`
            )
          }
