...
async function generateFiftyTawiPDF(whtData: any, companyData: any): Promise<Buffer> {
  ...
  return new Promise((resolve, reject) => {
    ...
    // ===== HEADER =====
    doc.font('ThaiBold').fontSize(16).text('หนังสือรับรองการหักภาษี ณ ที่จ่าย', 40, 40, { align: 'center', width: pageWidth })
    doc.font('Thai').fontSize(12).text('(ตามมาตรา 50 ทวิ แห่งประมวลรัษฎากร)', 40, 62, { align: 'center', width: pageWidth })
    ...
    doc.moveTo(col1, 148).lineTo(col1 + pageWidth, 148).stroke()
    ...
    doc.moveTo(col1, 243).lineTo(col1 + pageWidth, 243).stroke()
    ...
    doc.moveTo(col1, tableTop + 13).lineTo(col1 + pageWidth, tableTop + 13).stroke()
    ...
    doc.moveTo(col1, th + 14).lineTo(col1 + pageWidth, th + 14).stroke()
    ...
    doc.moveTo(col1, tr + 18).lineTo(col1 + pageWidth, tr + 18).stroke()

    // Total row
    const totalRow = tr + 26
    doc.font('ThaiBold').fontSize(11)
    doc.text('รวม', col1, totalRow, { width: 200 })
    doc.text(formatNumber(whtData.incomeAmount), col1 + 200, totalRow, { width: 120, align: 'right' })
    doc.text(formatNumber(whtData.whtAmount), col1 + 400, totalRow, { width: 100, align: 'right' })
    doc.moveTo(col1, totalRow + 16).lineTo(col1 + pageWidth, totalRow + 16).strokeColor('black').stroke()

    // ===== PND TYPE =====
    const pndY = totalRow + 30
    doc.font('Thai').fontSize(11)
    const pndText = whtData.type === 'PND3'
      ? '☑ ภ.ง.ด.3   ☐ ภ.ง.ด.53'
      : '☐ ภ.ง.ด.3   ☑ ภ.ง.ด.53'
    ...
    // ===== PAYMENT CONDITION =====
    const condY = pndY + 20
    doc.text('เงื่อนไขการชำระเงิน: ☑ หัก ณ ที่จ่าย   ☐ ออกให้ตลอดไป   ☐ ออกให้ครั้งเดียว', col1, condY)
    ...
    doc.text('* กรุณาเก็บเอกสารนี้ไว้เพื่อใช้ประกอบการยื่นแบบแสดงรายการภาษีเงินได้', col1, footY, { align: 'center', width: pageWidth })
    doc.text(`สร้างโดย Thai Accounting ERP — ${new Date().toLocaleString('th-TH')}`, col1, footY + 14, { align: 'center', width: pageWidth })
...
function formatThaiDate(date: Date | string): string {
  ...
  return d.toLocaleDateString('th-TH', { year: 'numeric', month: 'long', day: 'numeric', calendar: 'buddhist' } as any)
}
