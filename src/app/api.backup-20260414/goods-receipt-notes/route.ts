...
export async function GET(request: NextRequest) {
  try {
    ...
    const vendorIds = [...new Set(grns.map((g) => g.vendorId).filter((id): id is string => id !== null))]
    const warehouseIds = [...new Set(grns.map((g) => g.warehouseId).filter((id): id is string => id !== null))]
    ...
    const [vendors, warehouses] = await Promise.all([
      ...
      db.warehouse.findMany({
        where: warehouseIds.length > 0 ? { id: { in: warehouseIds } } : { id: { in: [] as string[] } },
...
export async function POST(request: NextRequest) {
  try {
    ...
    const message =
      error instanceof Error ? error.message : "เกิดข้อผิดพลาดในการสร้างใบรับสินค้า"
