...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!cheque) {
      return NextResponse.json({ success: false, error: 'ไม่พบเช็ค' }, { status: 404 });
    }
  ...
  try {
    ...
    if (status && (status === 'CLEARED' || status === 'BOUNCED')) {
      if (status === 'CLEARED') {
        ...
        const journalEntry = await clearCheque(id, date);
      ...
      if (status === 'BOUNCED') {
        ...
        const date = clearedDate ? new Date(clearedDate) : new Date();
        const journalEntry = await bounceCheque(id, date, bounceReason);
