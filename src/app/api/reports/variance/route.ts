...
export async function GET(request: NextRequest) {
  try {
    ...
    const minQtyVariancePct = minQtyVariancePctParam
      ? parseFloat(minQtyVariancePctParam)
      : undefined;
    const minPriceVariancePct = minPriceVariancePctParam
      ? parseFloat(minPriceVariancePctParam)
      : undefined;
    ...
    const invMap: Record<
      string,
      { id: string; invoiceNo: string; invoiceDate: Date; vendorId: string | null }
    > = Object.fromEntries(purchaseInvoices.map((i) => [i.id, i]));
    const vendorMap: Record<string, { id: string; code: string; name: string }> =
    ...
    const items = rawMatches
      .map((match) => {
        ...
        return {
          ...
          purchaseInvoice: inv
            ? { invoiceNo: inv.invoiceNo, invoiceDate: inv.invoiceDate, vendorId: inv.vendorId }
            : null,
          vendor,
        };
      })
    ...
    items.sort((a, b) => {
      const aWorst = Math.max(a.qtyVariancePct ?? 0, a.priceVariancePct ?? 0);
      const bWorst = Math.max(b.qtyVariancePct ?? 0, b.priceVariancePct ?? 0);
      return bWorst - aWorst;
    });

    // Build summary: byVendor
    const summaryVendorMap = new Map<
      string,
