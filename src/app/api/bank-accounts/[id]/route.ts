    ...
    if (isActive === false) {
      ...
      if (chequeCount > 0) {
        return NextResponse.json(
          {
            success: false,
            error: `ไม่สามารถระงับบัญชีได้ เนื่องจากมีเช็คที่ยังไม่ได้กระทบยอด ${chequeCount} รายการ`,
          },
          { status: 400 }
        );
      }
  ...
  try {
    ...
    if (reconCount > 0) {
      return NextResponse.json(
        {
          success: false,
          error: `ไม่สามารถลบบัญชีได้ เนื่องจากมีรายการกระทบยอด ${reconCount} รายการ`,
        },
        { status: 400 }
      );
    }
