...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!fundId || !payee || !description || !amount || !glExpenseAccountId) {
      return NextResponse.json({ success: false, error: 'ข้อมูลใบสำคัญไม่ครบถ้วน' }, { status: 400 })
    }

    const fund = await prisma.pettyCashFund.findUnique({ where: { id: fundId } })
    if (!fund) return NextResponse.json({ success: false, error: 'ไม่พบกองทุนเงินสดย่อย' }, { status: 404 })
