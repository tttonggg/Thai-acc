/**
 * Invoices API with Audit Logging
 * Demonstrates security hardening with comprehensive audit trails
 */
...
// POST - Create invoice with audit logging
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Log invoice creation to audit trail
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const beforeInvoice = await prisma.invoice.findUnique({
      where: { id },
      include: { lines: true },
    });
    ...
    // Update invoice
    const afterInvoice = await prisma.invoice.update({
      where: { id },
      data: body,
      include: { lines: true },
    });
