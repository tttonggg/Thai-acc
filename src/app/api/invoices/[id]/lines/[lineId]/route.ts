...
export async function PUT(
  ...
  try {
    ...
    const result = await db.$transaction(async (tx) => {
      ...
      if (user.role !== "ADMIN" && existingLine.invoice.createdById && existingLine.invoice.createdById !== user.id) {
      ...
      const quantityDiff = validatedData.quantity !== undefined
        ? validatedData.quantity - existingLine.quantity
        : undefined

      const unitPriceDiff = validatedData.unitPrice !== undefined
        ? validatedData.unitPrice - existingLine.unitPrice
        : undefined

      const discountDiff = validatedData.discount !== undefined
      ...
      if (validatedData.description !== undefined && validatedData.description !== existingLine.description) {
        changedFields.push("description")
      }
      if (validatedData.quantity !== undefined && validatedData.quantity !== existingLine.quantity) {
        changedFields.push("quantity")
      }
      if (validatedData.unitPrice !== undefined && validatedData.unitPrice !== existingLine.unitPrice) {
        changedFields.push("unitPrice")
      }
      if (validatedData.discount !== undefined && validatedData.discount !== existingLine.discount) {
    ...
    if (error.message === "INVOICE_NOT_DRAFT") {
      return apiError("ไม่สามารถแก้ไขรายการในใบกำกับภาษีที่ออกแล้วได้ - เฉพาะสถานะร่างเท่านั้น (Thai Tax Compliance)")
    }
    ...
    if (error instanceof Error && error.name === "ZodError") {
      return apiError("ข้อมูลไม่ถูกต้อง: " + JSON.parse(error.message)[0]?.message || "กรุณาตรวจสอบข้อมูล")
    }
...
export async function DELETE(
  ...
  try {
    ...
    if (error.message === "INVOICE_NOT_DRAFT") {
      return apiError("ไม่สามารถลบรายการในใบกำกับภาษีที่ออกแล้วได้ - เฉพาะสถานะร่างเท่านั้น (Thai Tax Compliance)")
    }
