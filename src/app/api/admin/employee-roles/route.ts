...
export async function GET() {
  try {
    ...
    return NextResponse.json({ success: false, error: 'Failed to fetch employee roles' }, { status: 500 })
  }
}
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (!id) {
      return NextResponse.json({ success: false, error: 'EmployeeRole ID is required' }, { status: 400 })
    }
