...
export function initIO(server: NetServer): SocketIOServer {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('acquire_lock', (data: { module: string; documentId: string; userId: string; userName: string }) => {
      const room = `${data.module}:${data.documentId}`
    ...
    socket.on('cursor_move', (data: { module: string; documentId: string; cursor: { x: number; y: number } }) => {
      const room = `${data.module}:${data.documentId}`
