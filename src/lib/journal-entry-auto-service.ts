...
async function createJournalEntry(
  ...
  if (totalDebit !== totalCredit)
    throw new Error(`Unbalanced JE: Dr=${totalDebit} Cr=${totalCredit}`);
...
export async function onInvoicePosted(invoiceId: string): Promise<CreateJEResult> {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const lines: JELine[] = [
      {
        ...
        description: `ลูกหนี้ - ${invoice.customer.name}`,
    ...
    const result = await createJournalEntry(tx, {
      ...
      description: `ขาย ${invoice.invoiceNo}`,
...
export async function onReceiptPosted(receiptId: string): Promise<CreateJEResult> {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const lines: JELine[] = [
      {
        ...
        description: `รับเงินจาก ${r.customer.name}`,
        debit: r.amount,
        credit: 0,
      },
      {
    ...
    const result = await createJournalEntry(tx, {
      date: r.receiptDate,
      description: `รับเงิน ${r.receiptNo}`,
...
export async function onPaymentPosted(paymentId: string): Promise<CreateJEResult> {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const lines: JELine[] = [
      ...
      {
        accountId: await getAccountId(tx, cbCode),
        description: `จ่ายเงิน ${p.paymentNo}`,
        debit: 0,
        credit: p.amount,
      },
    ];
    const result = await createJournalEntry(tx, {
      date: p.paymentDate,
      description: `จ่ายเงินให้ผู้ขาย ${p.vendor.name} เลขที่ ${p.paymentNo}`,
...
export async function onCreditNotePosted(creditNoteId: string): Promise<CreateJEResult> {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const result = await createJournalEntry(tx, {
      ...
      description: `ใบลดหนี้ ${cn.creditNoteNo} - ${cn.customer.name}`,
