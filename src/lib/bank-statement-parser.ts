...
export function parseCamt053Xml(xmlContent: string): ParseResult {
  try {
    ...
    const accountId =
      extractXmlValue(cleanXml, 'AcctId') || extractXmlValue(cleanXml, 'Othr > Id') || null;

    // Extract statement ID
    const statementId =
      extractXmlValue(cleanXml, 'Stmt > Id') || extractXmlValue(cleanXml, 'Id') || null;
...
function parseNtryBlock(ntryBlock: string): ParsedBankEntry | null {
  try {
    ...
    if (!description) {
      const rmtInfMatch = ntryBlock.match(
        /<RmtInf>[\s\S]*?<Ustrd>([^<]+)<\/Ustrd>[\s\S]*?<\/RmtInf>/
      );
