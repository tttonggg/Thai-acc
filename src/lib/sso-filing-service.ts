 ...
 */
export async function generate50TongFile(year: number, month: number): Promise<{
  content: string
  filename: string
}> {
...
export function getSSOSummary(filingData: SSOFilingData): {
  ...
  const avg = filingData.totalEmployees > 0
    ? (filingData.totalSSC / filingData.totalEmployees) / 100
    : 0
