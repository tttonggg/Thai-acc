 ...
 */
export function calculateNextRun(lastRunAt: Date, frequency: RecurringFrequency, dayOfMonth: number): Date {
  let nextDate: Date
 ...
 */
export async function processSingleDocument(recurring: any): Promise<{ documentId: string | null; error: string | null }> {
  try {
 ...
 */
async function createInvoiceFromRecurring(recurring: any): Promise<{ documentId: string; error: string | null }> {
  ...
  const newInvoice = await prisma.invoice.create({
    data: {
      invoiceNo,
      invoiceDate: now,
      dueDate: templateInvoice.dueDate ? new Date(now.getTime() + (templateInvoice.dueDate.getTime() - templateInvoice.invoiceDate.getTime())) : undefined,
 ...
 */
async function createExpenseFromRecurring(recurring: any): Promise<{ documentId: string; error: string | null }> {
  ...
  const newExpense = await prisma.purchaseInvoice.create({
    data: {
      invoiceNo,
      invoiceDate: now,
      dueDate: templateExpense.dueDate ? new Date(now.getTime() + (templateExpense.dueDate.getTime() - templateExpense.invoiceDate.getTime())) : undefined,
 ...
 */
async function createReceiptFromRecurring(recurring: any): Promise<{ documentId: string; error: string | null }> {
 ...
 */
export async function listRecurringDocuments(options: {
  type?: RecurringType
  isActive?: boolean
  page?: number
  limit?: number
