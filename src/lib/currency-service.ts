 ...
 */
export async function fetchLatestRates(): Promise<Array<{ code: string; rate: number }>> {
  try {
 ...
 */
export async function updateExchangeRatesFromAPI(createdBy?: string): Promise<number> {
...
export async function calculateRealizedGainLoss(
  ...
  const gainLossAmount = Math.round(foreignAmount * (currentRate - originalRate));
...
export async function generateMultiCurrencyReport(
  ...
  for (const currency of currencies) {
    ...
    const receivablesTHB = receivableInvoices.reduce((sum, inv) => sum + inv.totalAmount, 0);
    ...
    const realizedGainLoss = gainLoss.find((g) => g.type === 'REALIZED')?._sum.gainLossAmount || 0;
