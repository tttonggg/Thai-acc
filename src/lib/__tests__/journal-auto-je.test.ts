...
async function createTestAccount(code: string, name: string) {
  return prisma.chartOfAccount.upsert({
    where: { code },
    update: { name, type: code.startsWith('1') ? 'ASSET' : code.startsWith('2') ? 'LIABILITY' : code.startsWith('4') ? 'REVENUE' : 'EXPENSE', level: 4, isDetail: true, isActive: true },
    create: {
      code,
      name,
      type: code.startsWith('1') ? 'ASSET' : code.startsWith('2') ? 'LIABILITY' : code.startsWith('4') ? 'REVENUE' : 'EXPENSE',
      level: 4,
      isDetail: true,
      isActive: true,
    },
...
describe.skip('onGoodsReceiptPosted', () => {
  ...
  it('should throw error when GRN status is not RECEIVED', async () => {
    ...
    await expect(onGoodsReceiptPosted(grn.id)).rejects.toThrow(
      /สถานะ RECEIVED/
    )
  })
  ...
  it('should throw error when calling onGoodsReceiptPosted twice', async () => {
    ...
    await expect(onGoodsReceiptPosted(grn.id)).rejects.toThrow(
      /ถูกสร้างรายการบัญชีแล้ว/
    )
  })

  it('should throw error when GRN not found', async () => {
    await expect(onGoodsReceiptPosted('non-existent-id')).rejects.toThrow(
      /ไม่พบใบรับสินค้า/
    )
  })
})
...
describe('Double-post prevention', () => {
  ...
  it('should throw error when calling onCreditNotePosted twice', async () => {
    ...
    // Second call should throw
    await expect(onCreditNotePosted(cn.id)).rejects.toThrow(
      /ถูกสร้างรายการบัญชีแล้ว/
    )
  })
  ...
  it('should throw error when calling onReceiptPosted twice for same receipt', async () => {
    ...
    // Second call should throw
    await expect(onReceiptPosted(receipt.id)).rejects.toThrow(
      /ถูกสร้างรายการบัญชีแล้ว/
    )
  })
  ...
  it('should throw error when calling onPaymentPosted twice for same payment', async () => {
    ...
    // Second call should throw
    await expect(onPaymentPosted(payment.id)).rejects.toThrow(
      /ถูกสร้างรายการบัญชีแล้ว/
    )
  })
  ...
  it('should throw error when calling onPurchaseInvoicePosted twice', async () => {
    ...
    // Second call should throw
    await expect(onPurchaseInvoicePosted(pi.id)).rejects.toThrow(
      /ถูกสร้างรายการบัญชีแล้ว/
    )
  })
})
...
describe('Error handling', () => {
  it('should throw error when invoice not found', async () => {
    await expect(onInvoicePosted('non-existent-id')).rejects.toThrow(
      /ไม่พบใบกำกับภาษี/
    )
  })
  
  it('should throw error when receipt not found', async () => {
    await expect(onReceiptPosted('non-existent-id')).rejects.toThrow(
      /ไม่พบใบเสร็จรับเงิน/
    )
  })
  
  it('should throw error when payment not found', async () => {
    await expect(onPaymentPosted('non-existent-id')).rejects.toThrow(
      /ไม่พบใบจ่ายเงิน/
    )
  })
  
  it('should throw error when purchase invoice not found', async () => {
    await expect(onPurchaseInvoicePosted('non-existent-id')).rejects.toThrow(
      /ไม่พบใบซื้อ/
    )
  })
  
  it('should throw error when credit note not found', async () => {
    await expect(onCreditNotePosted('non-existent-id')).rejects.toThrow(
      /ไม่พบใบลดหนี้/
    )
  })
})
