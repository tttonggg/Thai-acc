...
describe('Payroll Service', () => {
  ...
  describe('createPayrollJournalEntry', () => {
    ...
    it('should throw error if payroll not found', async () => {
      ...
      await expect(createPayrollJournalEntry('nonexistent', 'user-1'))
        .rejects.toThrow('Payroll run nonexistent not found')
    })
    ...
    it('should throw error if payroll already has journal entry', async () => {
      ...
      await expect(createPayrollJournalEntry('payroll-1', 'user-1'))
        .rejects.toThrow('Payroll already has journal entry')
    })
