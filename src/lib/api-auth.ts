...
export async function requireAuth(request?: NextRequest): Promise<AuthUser> {
  // Bypass auth for testing in standalone mode
  if (
  ...
  const opts = request ? { ...authOptions, req: { headers: request.headers } } : authOptions;
 ...
 */
export function apiError(message: string, statusCode: number = 400, details?: any): NextResponse {
...
export function withAuth<T extends any[]>(
  ...
  return async (...args: T): Promise<NextResponse> => {
    try {
      const user = options?.roles ? await requireRole(options.roles) : await requireAuth();
