...
export async function recordStockMovement(params: {
  ...
  return await prisma.$transaction(async (tx) => {
    ...
    const product = existingBalance?.product ?? await tx.product.findUnique({ where: { id: productId } })
    ...
    const balance = await tx.stockBalance.upsert({
      where: { productId_warehouseId: { productId, warehouseId } },
      create: { productId, warehouseId, quantity: newQty, unitCost: newUnitCost, totalCost: newTotalCost },
...
export async function calculateCOGS(productId: string, warehouseId: string, quantity: number): Promise<number> {
