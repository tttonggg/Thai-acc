...
test.describe('AGENT_MODULES - Expansion Modules Test Suite', () => {
  ...
  test.describe('Inventory Module', () => {
    ...
    test('Inventory - Stock Balance Tab (สต็อกคงเหลือ)', async ({ page }) => {
      ...
      const summaryCards = page.locator('.grid.grid-cols-3 .card, .grid.grid-cols-3 [class*="border-l-4"]')
    ...
    test('Inventory - Stock Transfers Tab (โอนสต็อก)', async ({ page }) => {
      ...
      const transferBtn = page.locator('div[class*="content"] button:has-text("โอนสต็อก"), button:has-text("โอนสต็อก")').nth(1)
  ...
  test.describe('Banking Module', () => {
    ...
    test('Banking - Bank Accounts Tab (บัญชีธนาคาร)', async ({ page }) => {
      ...
      const accountsTab = page.locator('button:has-text("บัญชีธนาคาร"), nav button:has-text("บัญชี")').first()
    ...
    test('Banking - Cheque Register Tab (สมุดเช็ค)', async ({ page }) => {
      ...
      // Verify table headers
      const tableHeaders = ['เลขที่', 'ประเภท', 'ธนาคาร', 'ผู้รับ/จ่าย', 'ครบกำหนด', 'จำนวน', 'สถานะ']
      for (const header of tableHeaders) {
        const headerCell = page.locator(`th:has-text("${header}")`)
        const count = await headerCell.count()
        if (count > 0) {
  ...
  test.describe('Fixed Assets Module', () => {
    ...
    test('Assets - Asset List with Summary Cards', async ({ page }) => {
      ...
      // Verify table headers
      const tableHeaders = ['รหัส', 'ชื่อสินทรัพย์', 'วันที่ซื้อ', 'ราคาทุน', 'NBV', 'อายุ', 'สถานะ']
      for (const header of tableHeaders) {
        const headerCell = page.locator(`th:has-text("${header}")`)
        const count = await headerCell.count()
        if (count > 0) {
    ...
    test('Assets - Add Asset Dialog', async ({ page }) => {
      ...
      const dialogFields = ['รหัส', 'ชื่อสินทรัพย์', 'วันที่ซื้อ', 'ราคาทุน', 'ค่าซาก', 'อายุการใช้งาน']
  ...
  test.describe('Payroll Module', () => {
    ...
    test('Payroll - Payroll Runs Tab (รอบเงินเดือน)', async ({ page }) => {
      ...
      // Verify table headers
      const tableHeaders = ['เลขที่รอบ', 'งวด', 'จำนวนพนักงาน', 'ฐานเงินเดือน', 'ประกันสังคม', 'ภาษี PND1', 'เงินได้สุทธิ', 'สถานะ']
      for (const header of tableHeaders) {
        const headerCell = page.locator(`th:has-text("${header}")`)
        const count = await headerCell.count()
        if (count > 0) {
  ...
  test.describe('Petty Cash Module', () => {
    ...
    test('Petty Cash - Vouchers Tab (ใบสำคัญ)', async ({ page }) => {
      ...
      // Verify table headers
      const tableHeaders = ['เลขที่', 'วันที่', 'กองทุน', 'จ่ายให้', 'รายละเอียด', 'จำนวน', 'เบิกคืน']
      for (const header of tableHeaders) {
        const headerCell = page.locator(`th:has-text("${header}")`)
        const count = await headerCell.count()
        if (count > 0) {
