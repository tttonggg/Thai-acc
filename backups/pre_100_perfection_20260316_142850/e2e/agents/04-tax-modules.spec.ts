...
test.describe('Tax Modules (ภาษี)', () => {
  ...
  test.describe('WHT Module (ภาษีหัก ณ ที่จ่าย)', () => {
    ...
    test('WHT module loads with report tabs', async ({ page }) => {
      ...
      const reportTab = page.locator('button').filter({ hasText: /รายงาน ภ\.ง\.ด\./i }).first()
      ...
      const pnd53Tab = page.locator('button').filter({ hasText: /53|ห้าสาม/i }).first()
      const pnd3Tab = page.locator('button').filter({ hasText: /ภงด\.3|พีเอ็นดี.*3/i }).first()
    ...
    test('WHT module has period and type filters', async ({ page }) => {
      ...
      const selectors = page.locator('[role="combobox"], button:has-text("มกราคม"), button:has-text("256")')
      ...
      const exportBtn = page.locator('button').filter({ hasText: /ส่งออก/i }).first()
    ...
    test('WHT PND53 tab displays correct table columns', async ({ page }) => {
      ...
      const pnd53Tab = page.locator('button').filter({ hasText: /53|ห้าสาม|บริการ|เช่า/i }).first()
      ...
      if (hasTable) {
        ...
        for (let i = 0; i < headerCount; i++) {
          const text = await headers.nth(i).textContent().catch(() => 'no text')
          console.log(`Header ${i}: ${text}`)
        }
        
        // Check for key columns with flexible matching
        if (headerCount > 0) {
          await expect(page.locator('th').filter({ hasText: /หนังสือรับรอง|เลขที่/i }).first()).toBeVisible()
          await expect(page.locator('th').filter({ hasText: /ผู้ถูกหัก|ผู้รับ/i }).first()).toBeVisible()
        }
      }
    ...
    test('WHT PND3 tab displays correct table columns', async ({ page }) => {
      ...
      const pnd3Tab = page.locator('button').filter({ hasText: /ภงด\.3|เงินเดือน|ค่าจ้าง/i }).first()
      ...
      if (hasTable) {
        ...
        if (headerCount > 0) {
          ...
          await expect(page.locator('th').filter({ hasText: /รหัส|พนักงาน/i }).first()).toBeVisible()
          await expect(page.locator('th').filter({ hasText: /ชื่อ|นามสกุล/i }).first()).toBeVisible()
        }
      }
  ...
  test.describe('WHT Management (จัดการภาษีหัก ณ ที่จ่าย)', () => {
    ...
    test('WHT records table displays correct columns', async ({ page }) => {
      ...
      if (hasTable) {
        await expect(page.locator('th').filter({ hasText: /วันที่/i }).first()).toBeVisible()
        await expect(page.locator('th').filter({ hasText: /ผู้ถูกหัก|ผู้รับ/i }).first()).toBeVisible()
        await expect(page.locator('th').filter({ hasText: /ประเภทเงินได้|รายได้/i }).first()).toBeVisible()
        await expect(page.locator('th').filter({ hasText: /อัตราภาษี/i }).first()).toBeVisible()
        await expect(page.locator('th').filter({ hasText: /ภาษีที่หัก/i }).first()).toBeVisible()
      } else {
  ...
  test('generate tax module test summary', async ({ page }) => {
    ...
    const testResults = {
      vat: {
        ...
        columns: ['วันที่ (Date)', 'เลขที่เอกสาร (Document No)', 'คู่ค้า (Partner)', 'มูลค่าสินค้า/บริการ (Amount)', 'ภาษีมูลค่าเพิ่ม (VAT Amount)'],
        chart: 'กราฟเปรียบเทียบภาษีขาย-ภาษีซื้อ'
      },
      wht: {
        tabs: ['PND53 (ภงด.53) - ค่าบริการ/ค่าเช่า', 'PND3 (ภงด.3) - เงินเดือน/ค่าจ้าง'],
        summaryCards: ['ภงด.3 (เงินเดือน/ค่าจ้าง)', 'ภงด.53 (ค่าบริการ/ค่าเช่า)', 'รวมภาษีที่ต้องยื่น'],
        periodSelector: ['Month Selector', 'Year Selector', 'Print Button', 'Export Button'],
        pnd53Columns: ['วันที่ (Date)', 'เลขที่หนังสือรับรอง (WHT Certificate No)', 'ผู้ถูกหักภาษี (Payee)', 'ประเภทเงินได้ (Income Type)', 'จำนวนเงินที่จ่าย (Payment Amount)', 'อัตราภาษี (Tax Rate)', 'ภาษีที่หัก (WHT Amount)'],
        pnd3Columns: ['วันที่ (Date)', 'เลขที่หนังสือรับรอง (WHT Certificate No)', 'พนักงาน (Employee)', 'เงินได้ (Income)', 'อัตราภาษี (Tax Rate)', 'ภาษีที่หัก (WHT Amount)']
      }
    }
