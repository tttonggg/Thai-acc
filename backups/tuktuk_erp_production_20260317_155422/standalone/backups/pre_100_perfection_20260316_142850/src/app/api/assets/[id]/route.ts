...
export async function GET(
  ...
  try {
    ...
    return NextResponse.json({ success: false, error: 'เกิดข้อผิดพลาดในการดึงข้อมูลสินทรัพย์' }, { status: 500 })
  }
}
...
export async function PUT(
  ...
  try {
    ...
    if (user.role === 'VIEWER') {
      return NextResponse.json({ success: false, error: 'ไม่มีสิทธิ์แก้ไขสินทรัพย์' }, { status: 403 })
    }
    ...
    if (code && code !== existing.code) {
      ...
      if (duplicate) {
        return NextResponse.json({ success: false, error: 'รหัสสินทรัพย์นี้มีอยู่แล้ว' }, { status: 400 })
      }
    }
    ...
    if (purchaseCost !== undefined && !hasPostedDepreciation) updateData.purchaseCost = parseFloat(purchaseCost)
    if (salvageValue !== undefined && !hasPostedDepreciation) updateData.salvageValue = parseFloat(salvageValue)
    ...
    if (!hasPostedDepreciation && (purchaseCost !== undefined || salvageValue !== undefined || usefulLifeYears !== undefined)) {
    ...
    return NextResponse.json({ success: false, error: 'ไม่สามารถอัปเดตข้อมูลสินทรัพย์ได้' }, { status: 500 })
  }
}
