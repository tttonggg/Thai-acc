...
async function main() {
  ...
  console.log(`📊 Found ${customers.length} customers, ${vendors.length} vendors, ${products.length} products`)
  ...
  const unpaidInvoices = invoicesCreated.filter(inv =>
  ...
  for (let i = 0; i < 150; i++) {
    ...
    const refInvoice = unpaidInvoices.length > 0
      ? unpaidInvoices[randomInt(0, unpaidInvoices.length - 1)]
      : null
