...
export async function generateThaiTestPDF(): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      // Title
      doc.fontSize(20)
         .text('ทดสอบฟอนต์ภาษาไทย / Thai Font Test', { align: 'center' })
         .moveDown(2)

      // Test different font sizes
      doc.fontSize(14)
         .text('นี่คือการทดสอบฟอนต์ภาษาไทยด้วย PDFKit', { align: 'left' })
         .moveDown()
      ...
      doc.text(`จำนวนเงิน: ${formatCurrency(testAmount)} บาท`)
         .moveDown()

      // Test date
      doc.text(`วันที่: ${formatDateThai(new Date())}`)
         .moveDown(2)
      ...
      tableData.forEach((row, i) => {
        ...
        doc.text(row[2], x + colWidths.col1 + colWidths.col2 + 30, y, { width: colWidths.col3, align: 'right' })
      ...
      doc.font(boldFontPath)
         .fontSize(11)
         .text(`รวมทั้งสิ้น / Total: ${formatCurrency(850.00)} บาท`, 60, doc.y, { align: 'right' })
         .moveDown(2)
...
export async function generateInvoicePDFWithPDFKit(invoice: any): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      doc.text('จำนวน', tableLeft + colWidths[0] + colWidths[1], headerY, { width: colWidths[2], align: 'center' })
      doc.text('หน่วย', tableLeft + colWidths[0] + colWidths[1] + colWidths[2], headerY, { width: colWidths[3], align: 'center' })
      doc.text('ราคา/หน่วย', tableLeft + colWidths[0] + colWidths[1] + colWidths[2] + colWidths[3], headerY, { width: colWidths[4], align: 'right' })
      doc.text('ส่วนลด', tableLeft + colWidths[0] + colWidths[1] + colWidths[2] + colWidths[3] + colWidths[4], headerY, { width: colWidths[5], align: 'right' })
      doc.text('จำนวนเงิน', tableLeft + colWidths[0] + colWidths[1] + colWidths[2] + colWidths[3] + colWidths[4] + colWidths[5], headerY, { width: colWidths[6], align: 'right' })

      // Header line
      yPos += 12
      doc.moveTo(tableLeft, yPos)
         .lineTo(tableRight, yPos)
         .lineWidth(0.5)
         .stroke()
      yPos += 6
      ...
      invoice.lines.forEach((line: any) => {
        ...
        doc.text(line.quantity.toString(), tableLeft + colWidths[0] + colWidths[1], yPos, { width: colWidths[2], align: 'center' })
        doc.text(line.unit, tableLeft + colWidths[0] + colWidths[1] + colWidths[2], yPos, { width: colWidths[3], align: 'center' })
        doc.text(formatCurrency(line.unitPrice), tableLeft + colWidths[0] + colWidths[1] + colWidths[2] + colWidths[3], yPos, { width: colWidths[4], align: 'right' })
        doc.text(line.discount > 0 ? formatCurrency(line.discount) : '-', tableLeft + colWidths[0] + colWidths[1] + colWidths[2] + colWidths[3] + colWidths[4], yPos, { width: colWidths[5], align: 'right' })
        doc.text(formatCurrency(line.amount), tableLeft + colWidths[0] + colWidths[1] + colWidths[2] + colWidths[3] + colWidths[4] + colWidths[5], yPos, { width: colWidths[6], align: 'right' })
        yPos += 16
      })
      ...
      if (invoice.discountAmount > 0) {
        ...
        doc.text(`(${formatCurrency(invoice.discountAmount)})`, tableRight, yPos, { align: 'right' })
        yPos += 14
      }
      ...
      if (invoice.notes || !invoice.terms) {
        doc.font(regularFontPath)
           .fontSize(11)
           .text('เงื่อนไข / Terms & Conditions:', margin, yPos)
        yPos += 15
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      if (receipt.notes) {
        doc.font(regularFontPath)
           .fontSize(10)
           .text('หมายเหตุ / Notes:', margin, yPos)
        yPos += 15
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      const deductionsData = [
        ...
        ['', ''],
        ['รวมหัก / Total Deductions', formatCurrency(
