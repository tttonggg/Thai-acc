...
export function CustomerListVirtual({
  ...
  const { selectedIds, selectedCount, toggleSelection, selectAll, clearSelection, isSelected } = useBulkSelection(customers)
  ...
  const columns: VirtualTableColumn<Customer>[] = [
    ...
    },
    {
      ...
      sortable: true,
      formatter: (value) => (
        <Badge variant={value === 'active' ? 'default' : 'secondary'} className={value === 'active' ? 'bg-green-100 text-green-800' : ''}>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex justify-between items-center text-sm text-muted-foreground">
        <span>แสดง {filteredCustomers.length} จาก {customers.length} รายการ</span>
      </div>
    </div>
  )
}
