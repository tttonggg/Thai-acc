...
export default function EditPurchaseOrderPage() {
  ...
  const updatePO = useMutation({
    ...
    onSuccess: () => {
      ...
      router.push(`/expenses/purchase-orders/${poId}`);
    },
  });
  ...
  useEffect(() => {
    if (po) {
      ...
      setVatRate(po.vat_rate?.toString() || "7");
      setDiscountAmount(po.discount_amount?.toString() || "0");
      setNotes(po.notes || "");
      setItems(
  ...
  if (!po) {
    return (
      <AppLayout>
        <div className="p-8 text-center">ไม่พบใบสั่งซื้อ</div>
      </AppLayout>
    );
  }
      ...
      <AppLayout>
        <div className="p-8 max-w-3xl">
          <div className="flex items-center gap-4 mb-8">
            <Link href={`/expenses/purchase-orders/${poId}`} className="text-gray-500 hover:text-gray-700">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8 text-center">
            ...
            <p className="text-gray-500">
              ใบสั่งซื้อนี้อยู่ในสถานะ {po.status} ไม่สามารถแก้ไขได้ (แก้ไขได้เฉพาะสถานะร่าง)
            </p>
            <Link
              href={`/expenses/purchase-orders/${poId}`}
    ...
    <AppLayout>
      <div className="p-8 max-w-5xl">
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">แก้ไขใบสั่งซื้อ</h1>
            <p className="text-gray-500 mt-1">{po.po_number}</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex justify-end gap-4">
            ...
            <button
              type="submit"
              disabled={updatePO.isPending || !contactId}
            ...
            >
              {updatePO.isPending ? "กำลังบันทึก..." : "บันทึก"}
            </button>
          </div>
        </form>
      </div>
