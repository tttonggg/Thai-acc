...
export default function EditPurchaseInvoicePage() {
  ...
  const { data: pi, isLoading } = useQuery({
    queryKey: ["purchase-invoice", piId],
    queryFn: () => api.get(`/purchase-invoices/${piId}`).then((res) => res.data),
  });

  const updatePI = useMutation({
    mutationFn: (data: any) => api.put(`/purchase-invoices/${piId}`, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["purchase-invoices"] });
      queryClient.invalidateQueries({ queryKey: ["purchase-invoice", piId] });
      router.push(`/expenses/purchase-invoices/${piId}`);
    },
  });
  ...
  useEffect(() => {
    if (pi) {
      setContactId(pi.contact_id || "");
      setProjectId(pi.project_id || "");
      setBillDate(pi.bill_date || "");
      setDueDate(pi.due_date || "");
      setVatRate(pi.vat_rate?.toString() || "7");
      setDiscountAmount(pi.discount_amount?.toString() || "0");
      setNotes(pi.notes || "");
      setItems(
  ...
  if (!pi) {
    return (
      <AppLayout>
        <div className="p-8 text-center">ไม่พบใบแจ้งหนี้ซื้อ</div>
      </AppLayout>
    );
  }

  if (!["draft", "received"].includes(pi.status)) {
    return (
      <AppLayout>
        <div className="p-8 max-w-3xl">
          <div className="flex items-center gap-4 mb-8">
            <Link href={`/expenses/purchase-invoices/${piId}`} className="text-gray-500 hover:text-gray-700">
              <ArrowLeft className="w-5 h-5" />
            </Link>
            <h1 className="text-2xl font-bold text-gray-900">แก้ไขใบแจ้งหนี้ซื้อ</h1>
          </div>
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8 text-center">
            <Lock className="w-12 h-12 text-gray-400 mx-auto mb-4" />
            <h2 className="text-lg font-semibold text-gray-900 mb-2">ไม่สามารถแก้ไขได้</h2>
            <p className="text-gray-500">
              ใบแจ้งหนี้ซื้อนี้อยู่ในสถานะ {pi.status} ไม่สามารถแก้ไขได้
            </p>
            <Link
              href={`/expenses/purchase-invoices/${piId}`}
    ...
    <AppLayout>
      <div className="p-8 max-w-5xl">
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">แก้ไขใบแจ้งหนี้ซื้อ</h1>
            <p className="text-gray-500 mt-1">{pi.bill_number}</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex justify-end gap-4">
            ...
            <button
              type="submit"
              disabled={updatePI.isPending || !contactId}
            ...
            >
              {updatePI.isPending ? "กำลังบันทึก..." : "บันทึก"}
            </button>
          </div>
        </form>
      </div>
