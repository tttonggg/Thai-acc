...
import { useContacts, useProjects, useQuotations, useCreateInvoice } from "@/hooks/useApi";
...
export default function InvoiceForm() {
  ...
  const [quotationId, setQuotationId] = useState(prefillQuotationId || "");
    ...
    <AppLayout>
      <div className="p-8 max-w-5xl">
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">สร้างใบแจ้งหนี้</h1>
            <p className="text-gray-500 mt-1">กรอกข้อมูลเพื่อสร้างใบแจ้งหนี้ใหม่</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">ใบเสนอราคา (ถ้ามี)</label>
                <select
                  value={quotationId}
                ...
                >
                  ...
                  {quotations?.filter((q: any) => q.status === "accepted").map((q: any) => (
                    <option key={q.id} value={q.id}>{q.quotation_number} - {q.contact_name}</option>
                  ))}
                </select>
              </div>
              <div>
          ...
          <div className="flex justify-end gap-4">
            ...
            <button type="submit" disabled={isSubmitting || !contactId} className="px-6 py-2.5 bg-gradient-to-r from-peak-purple to-peak-teal text-white rounded-lg font-medium hover:opacity-90 transition-opacity disabled:opacity-50 disabled:cursor-not-allowed">
              {isSubmitting ? "กำลังบันทึก..." : "สร้างใบแจ้งหนี้"}
            </button>
          </div>
        </form>
      </div>
